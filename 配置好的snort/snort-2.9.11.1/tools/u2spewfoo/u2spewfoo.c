...
static int get_record(u2iterator *it, u2record *record) {
    ...
    bytes_read = fread(record, 1, sizeof(u_int32_t) * 2, it->file);
    ...
    if(bytes_read != sizeof(u_int32_t)*2) {
        puts("get_record: (1) Failed to read all of record.");
        printf("\tRead %u of %lu bytes\n", bytes_read, (unsigned long)sizeof(u_int32_t)*2);
        return FAILURE;
    }
...
static void extradata_dump(u2record *record) {
    ...
    for(i=0; i<2; i++, field+=4) {
        *(u_int32_t*)field = ntohl(*(u_int32_t*)field);
    }
    ...
    switch(event.type)
    {
        case EVENT_INFO_XFF_IPV4:
            memcpy(&ip, record->data + sizeof(Unified2ExtraDataHdr) + sizeof(SerialUnified2ExtraData), sizeof(u_int32_t));
            ip = ntohl(ip);
...
static void event2_dump(u2record *record) {
    ...
    *(uint16_t*)field = ntohs(*(uint16_t*)field); /* dport_icode */
    field +=6;
    *(u_int32_t*)field = ntohl(*(u_int32_t*)field); /* mpls_label */
    field += 4;
...
static void appid_dump(u2record *record) {
    ...
    /* In the event structure, only the last 40 bits are not 32 bit fields */
    /* The first fields need to be convertted */
    statTime = ntohl(*(u_int32_t*)field);
    field += 4;
    appCnt = ntohl(*(u_int32_t*)field); 
    field += 4;
    ...
    for(i=0; i<appCnt; i++) 
    {
        ...
        int txBytes = ntohl(*(u_int32_t*)field);
        field += 4;
        int rxBytes = ntohl(*(u_int32_t*)field);
        field += 4;
