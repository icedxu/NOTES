...
static int pcap_daq_open(Pcap_Context_t *context)
{
    ...
    u_int32_t defaultnet = 0xFFFFFF00;
#ifndef PCAP_OLDSTYLE
    int status;
#endif /* PCAP_OLDSTYLE */
...
static int pcap_daq_inject(void *handle, const DAQ_PktHdr_t *hdr, const uint8_t *packet_data, u_int32_t len, int reverse)
{
    Pcap_Context_t *context = (Pcap_Context_t *) handle;
...
static u_int32_t pcap_daq_get_capabilities(void *handle)
{
    Pcap_Context_t *context = (Pcap_Context_t *) handle;
    u_int32_t capabilities = DAQ_CAPA_BPF;
