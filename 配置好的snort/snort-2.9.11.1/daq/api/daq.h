...
typedef struct {
    ...
    u_int32_t version;   /* Module version */
    u_int32_t type;      /* Module capabilities */
} DAQ_Module_Info_t;
...
DAQ_LINKAGE u_int32_t daq_get_type(const DAQ_Module_t *module);
...
DAQ_LINKAGE int daq_acquire_with_meta(const DAQ_Module_t *module, void *handle, int cnt,
                                      ...
                                      void *user);
DAQ_LINKAGE int daq_inject(const DAQ_Module_t *module, void *handle, const DAQ_PktHdr_t *hdr, const uint8_t *packet_data, u_int32_t len, int reverse);
...
DAQ_LINKAGE u_int32_t daq_get_capabilities(const DAQ_Module_t *module, void *handle);
