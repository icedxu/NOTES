...
typedef struct _daq_pkthdr
{
    ...
    u_int32_t caplen;        /* Length of the portion present */
    u_int32_t pktlen;        /* Length of this packet (off wire) */
    ...
    u_int32_t flags;         /* Flags for the packet (DAQ_PKT_FLAG_*) */
    u_int32_t opaque;        /* Opaque context value from the DAQ module or underlying hardware.
...
typedef struct _daq_config
{
    ...
    u_int32_t flags;     /* Other configuration flags (DAQ_CFG_*) */
...
typedef struct _DAQ_DP_key_t {
    u_int32_t af;                /* AF_INET or AF_INET6 */
    union {
