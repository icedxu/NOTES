...
u_int32_t Byte_Extract_Offset_Var;

/* Storage for extracted variables */
static u_int32_t extracted_values[NUM_BYTE_EXTRACT_VARS];
...
u_int32_t ByteExtractHash(void *d)
{
    u_int32_t a,b,c;
...
int numBytesInBitmask(u_int32_t bitmask_value)
{
   int num_bytes;
...
void RuleOptionBitmaskParse(u_int32_t* bitmask_val, char *cptr, u_int32_t bytes_to_extract,char* ruleOptionName)
{
     char* endp = NULL;
     char* startp = (cptr + 8);
     u_int32_t bitmask_value;
...
u_int32_t getNumberTailingZerosInBitmask(u_int32_t bitmask)
{
...
int GetByteExtractValue(u_int32_t *dst, int8_t var_number)
{
...
int SetByteExtractValue(u_int32_t value, int8_t var_number)
{
