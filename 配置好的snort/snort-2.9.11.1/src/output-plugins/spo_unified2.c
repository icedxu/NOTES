...
static void Unified2Write(uint8_t *, u_int32_t, Unified2Config *);
...
static void AlertExtraData(void *ssnptr, void *data, LogFunction *log_funcs, u_int32_t max_count, u_int32_t xtradata_mask, u_int32_t event_id, u_int32_t event_second);
...
static void Unified2InitFile(Unified2Config *config)
{
    ...
    config->timestamp = (u_int32_t)time(NULL);
...
static void _AlertIP4(Packet *p, const char *msg, Unified2Config *config, Event *event)
{
    ...
    u_int32_t write_len = sizeof(Serial_Unified2_Header) + sizeof(Serial_Unified2IDSEvent_legacy);
...
static void _AlertIP4_v2(Packet *p, const char *msg, Unified2Config *config, Event *event)
{
    ...
    u_int32_t write_len = sizeof(Serial_Unified2_Header) + sizeof(Unified2IDSEvent);
...
static void _AlertIP6(Packet *p, const char *msg, Unified2Config *config, Event *event)
{
    ...
    u_int32_t write_len = sizeof(Serial_Unified2_Header) + sizeof(Serial_Unified2IDSEventIPv6_legacy);
...
static void _AlertIP6_v2(Packet *p, const char *msg, Unified2Config *config, Event *event)
{
    ...
    u_int32_t write_len = sizeof(Serial_Unified2_Header) + sizeof(Unified2IDSEventIPv6);
...
void _WriteExtraData(Unified2Config *config, u_int32_t event_id, u_int32_t event_second, uint8_t *buffer, u_int32_t len, u_int32_t type )
{
...
static void AlertExtraData(
    ...
    LogFunction *log_funcs, u_int32_t max_count,
...
static void Unified2LogAlert(Packet *p, const char *msg, void *arg, Event *event)
{
    ...
    if ( p->xtradata_mask )
    {
        ...
        u_int32_t max_count = stream_api->get_xtra_data_map(&log_funcs);
                                    ...
                                    Unified2Config *config, Event *event)
{
    Serial_Unified2_Header hdr;
    Serial_Unified2Packet logheader;
    u_int32_t pkt_length = 0;
    u_int32_t write_len = sizeof(Serial_Unified2_Header) + sizeof(Serial_Unified2Packet) - 4;
        ...
        ob_char_t ob_char, void *userdata)
{
    ...
    {
        Serial_Unified2_Header hdr;
        u_int32_t record_len = (pkth->caplen + sizeof(Serial_Unified2_Header)
                + (sizeof(Serial_Unified2Packet) - 4));
 ...
 *
 ******************************************************************************/
static void Unified2Write(uint8_t *buf, u_int32_t buf_len, Unified2Config *config)
{
    size_t fwcount = 0;
    int ffstatus = 0;
