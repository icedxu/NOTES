...
void DecoderAlertEncapsulated(
    Packet *p, int type, const char *str, const uint8_t *pkt, u_int32_t len)
{
...
u_int32_t EXTRACT_32BITS (u_char *p)
{
  u_int32_t __tmp;
...
void UpdateDecodeRulesArray(u_int32_t sid, int value, int all_rules)
{
    int i;
...
static inline void PushLayer(PROTO_ID type, Packet* p, const uint8_t* hdr, u_int32_t len)
{
...
/*
 * Function: DecodeARP(uint8_t *, u_int32_t, Packet *)
 *
 ...
 */
void DecodeARP(const uint8_t * pkt, u_int32_t len, Packet * p)
{
    pc.arp++;
...
{
    u_int32_t cap_len = pkthdr->caplen;
    PROFILE_VARS;
...
/*
 * Function: DecodeEthLoopback(uint8_t *, u_int32_t)
 *
 ...
 * Returns: void function
 */
void DecodeEthLoopback(const uint8_t *pkt, u_int32_t len, Packet *p)
{
...
// decode.c::Ethernet
//--------------------------------------------------------------------
void DecodeEthTypes(Packet *p, const uint8_t *pkt, uint16_t ethtype, u_int32_t cap_len, uint8_t linklen);

void DecodeCiscoMeta(const uint8_t *pkt, u_int32_t rem_len, Packet *p)
{
    uint16_t real_len;
    uint16_t realeth;
...
#ifdef GRE
/*
 * Function: DecodeTransBridging(uint8_t *, const u_int32_t, Packet)
 *
 ...
 */
void DecodeTransBridging(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
    pc.gre_eth++;
 ...
 */
static int checkMplsHdr(u_int32_t label, uint8_t exp, uint8_t bos, uint8_t ttl, Packet *p)
{
    int iRet = 0;
    switch(label)
    {
...
void DecodeMPLS(const uint8_t* pkt, const u_int32_t len, Packet* p)
{
...
void DecodeEthOverMPLS(const uint8_t* pkt, const u_int32_t len, Packet* p)
{
    /* do a little validation */
    if(len < ETHERNET_HEADER_LEN)
    {
...
void DecodeVlan(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
    VlanTagHdr* vh;
 ...
 *
 */
void DecodePPPoEPkt(const uint8_t* pkt, const u_int32_t len, Packet* p)
{
...
/*
 * Function: DecodePppPktEncapsulated(Packet *, const u_int32_t len, uint8_t*)
 *
 ...
 * Returns: void function
 */
void DecodePppPktEncapsulated(const uint8_t* pkt, const u_int32_t len, Packet* p)
{
    static int had_vj = 0;
    uint16_t protocol;
...
static inline void ICMP4AddrTests (Packet* p)
{
    ...
    u_int32_t dst = sfaddr_get_ip4_value(GET_DST_IP(p));
...
static inline void DecodeIPv4Proto(const uint8_t proto,
    const uint8_t *pkt, const u_int32_t len, Packet *p)
{
    switch(proto)
    {
        case IPPROTO_TCP:
...
/*
 * Function: DecodeIP(uint8_t *, const u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeIP(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
...
/*
 * Function: DecodeICMP(uint8_t *, const u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeICMP(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
...
/*
 * Function: DecodeICMPEmbeddedIP(uint8_t *, const u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeICMPEmbeddedIP(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
...
/*
 * Function: DecodeIPV6(uint8_t *, u_int32_t)
 *
...
static inline void IPV6MiscTests(Packet *p)
{
    ...
    if ( Event_Enabled(DECODE_IPV6_ISATAP_SPOOF) )
    {
        ...
        {
            u_int32_t isatap_interface_id = ntohl(hdr6->ip6_src.s6_addr32[2]) & 0xFCFFFFFF;
...
void DecodeIPV6Extensions(uint8_t next, const uint8_t *pkt, u_int32_t len, Packet *p);

static inline int CheckIPV6HopOptions(const uint8_t *pkt, u_int32_t len, Packet *p)
{
    IP6Extension *exthdr = (IP6Extension *)pkt;
    u_int32_t total_octets = (exthdr->ip6e_len * 8) + 8;
...
void DecodeIPV6Options(int type, const uint8_t *pkt, u_int32_t len, Packet *p)
{
    IP6Extension *exthdr;
    u_int32_t hdrlen = 0;
...
void DecodeIPV6(const uint8_t *pkt, u_int32_t len, Packet *p)
{
    IP6RawHdr *hdr;
...
void DecodeICMP6(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
...
/*
 * Function: DecodeICMPEmbeddedIP6(uint8_t *, const u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeICMPEmbeddedIP6(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
    uint16_t orig_frag_offset;
...
/* Function: DecodeTeredo(uint8_t *, u_int32_t, Packet *)
 *
...
void DecodeTeredo(const uint8_t *pkt, u_int32_t len, Packet *p)
{
    ...
    if (ntohs(*(uint16_t *)pkt) == TEREDO_INDICATOR_AUTH)
    {
        ...
        if (len < (u_int32_t)(TEREDO_INDICATOR_AUTH_MIN_LEN + client_id_length + auth_data_length))
            return;
 ...
 *
 */
void DecodeAH(const uint8_t *pkt, u_int32_t len, Packet *p)
{
...
/*
 * Function: DecodeESP(const uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeESP(const uint8_t *pkt, u_int32_t len, Packet *p)
{
...
/*
 * Function: DecodeERSPANType2(uint8_t *, u_int32_t, Packet *)
 *
 ...
 *
 */
void DecodeERSPANType2(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
    u_int32_t hlen = sizeof(ERSpanType2Hdr);
    u_int32_t payload_len;
...
/*
 * Function: DecodeERSPANType3(uint8_t *, u_int32_t, Packet *)
 *
 ...
 *
 */
void DecodeERSPANType3(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
    u_int32_t hlen = sizeof(ERSpanType3Hdr);
    u_int32_t payload_len;
...
/*
 * Function: DecodeGRE(uint8_t *, u_int32_t, Packet *)
 *
 ...
 */
void DecodeGRE(const uint8_t *pkt, const u_int32_t len, Packet *p)
{
...
/* Function: DecodeGTP(uint8_t *, u_int32_t, Packet *)
 *
...
void DecodeGTP(const uint8_t *pkt, u_int32_t len, Packet *p)
{
...
/*
 * Function: DecodeUDP(uint8_t *, const u_int32_t, Packet *)
 *
...
void DecodeUDP(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
    uint16_t uhlen;
...
/*
 * Function: DecodeTCP(uint8_t *, const u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeTCP(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
...
/*
 * Function: DecodeTCPOptions(uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeTCPOptions(const uint8_t *start, u_int32_t o_len, Packet *p)
{
...
/*
 * Function: DecodeIPOptions(uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeIPOptions(const uint8_t *start, u_int32_t o_len, Packet *p)
{
    const uint8_t *option_ptr = start;
...
/*
 * Function: DecodeEapol(uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeEapol(const uint8_t * pkt, u_int32_t len, Packet * p)
{
...
/*
 * Function: DecodeEapolKey(uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeEapolKey(const uint8_t * pkt, u_int32_t len, Packet * p)
{
...
/*
 * Function: DecodeEAP(uint8_t *, u_int32_t, Packet *)
 *
 ...
 * Returns: void function
 */
void DecodeEAP(const uint8_t * pkt, const u_int32_t len, Packet * p)
{
    p->eaph = (EAPHdr *) pkt;
...
/*
 * Function: DecodeIPX(uint8_t *, u_int32_t)
 *
 ...
 *
 */
void DecodeIPX(const uint8_t *pkt, u_int32_t len, Packet *p)
{
