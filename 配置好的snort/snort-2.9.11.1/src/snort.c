...
const u_int32_t FLOW_COUNT = (AAB_THRESHOLD/TIME_TO_PRUNE_ONE_SESSION) / 3;
...
u_int32_t *netmasks = NULL;   /* precalculated netmask array */
...
static int IsProcessingPackets(uint16_t type, const uint8_t *data, u_int32_t length, void **new_config,
...
int SnortMain(int argc, char *argv[])
{
	...
	if (!exit_signal){
		LogMessage("[PT] PacketLoop\n");
		PacketLoop();
	}
	LogMessage("[PT] CleanExit\n");
    ...
    void* user, const DAQ_PktHdr_t* pkthdr, const uint8_t* pkt)
{
...
#endif
	LogMessage("[PT] CheckForReload\n");
    CheckForReload();
	LogMessage("[PT] CheckForReload over\n");
    /* Save off the time of each and every packet */
    packet_time_update(&pkthdr->ts);
	LogMessage("[PT] packet_time_update over\n");
#ifdef REG_TEST
    ...
    {
        PREPROC_PROFILE_END_PI(totalPerfStats);
		LogMessage("[PT] ScTerminateService() || ScPauseService()\n");
        return verdict;  // time to go
    }
#endif
	LogMessage("[PT] sfthreshold_reset\n");
    /* reset the thresholding subsystem checks for this packet */
    sfthreshold_reset();
	LogMessage("[PT] PREPROC_PROFILE_START\n");
    PREPROC_PROFILE_START(eventqPerfStats);
	LogMessage("[PT] SnortEventqReset\n");
    SnortEventqReset();
    Replace_ResetQueue();
...
void PacketLoop (void)
{
    ...
    if ( !exit_logged && error )
    {
        ...
        {
			LogMessage("[PT] DAQ_Abort err_code: %d\n", error);
            DAQ_Abort();
            exit(1);
        }
		LogMessage("[PT] CleanExit err_code: %d\n", error);
        CleanExit(error);
    }
    done_processing = 1;
}
...
{
    if (netmasks == NULL)
        netmasks = (u_int32_t *)SnortAlloc(33 * sizeof(u_int32_t));
