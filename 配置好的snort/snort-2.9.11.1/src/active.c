...
static int Active_SendEth(const DAQ_PktHdr_t*, int, const uint8_t*, u_int32_t);
static int Active_SendIp(const DAQ_PktHdr_t*, int, const uint8_t*, u_int32_t);

typedef int (*send_t) (
    const DAQ_PktHdr_t* h, int rev, const uint8_t* buf, u_int32_t len);
...
static inline u_int32_t GetFlags (void)
{
    u_int32_t flags = ENC_FLAG_ID;
...
static u_int32_t Strafe(int, u_int32_t, const Packet*);

void Active_SendReset(Packet* p, EncodeFlags ef)
{
    int i;
    u_int32_t flags = (GetFlags() | ef) & ~ENC_FLAG_VAL;
    u_int32_t value = ef & ENC_FLAG_VAL;
...
bool Active_SendData (
    Packet* p, EncodeFlags flags, const uint8_t* buf, u_int32_t blen)
{
    uint16_t toSend;
    uint16_t sent;
    uint16_t maxPayload;
...
static u_int32_t Strafe (int i, u_int32_t flags, const Packet* p)
{
    flags &= ENC_FLAG_VAL;
