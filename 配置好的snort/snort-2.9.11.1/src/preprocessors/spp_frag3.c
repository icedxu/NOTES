...
static u_int32_t old_static_frags;
...
static u_int32_t Frag3MemReloadAdjust(unsigned);
...
char *FragIPToStr(u_int32_t ip[4], uint8_t proto)
{
    char *ret_str;
    sfaddr_t srcip;
...
static void Frag3PostConfigInit(struct _SnortConfig *sc, void *arg)
{
    ...
    if(config->use_prealloc)
    {
        ...
        {
            config->static_frags = (u_int32_t)(config->memcap /
...
static void Frag3ParseArgs(struct _SnortConfig *sc, char *args, Frag3Context *context)
{
    ...
    while(i < num_toks)
    {
        ...
        if(!strcasecmp(index, "timeout"))
        {
            ...
            increment = 2;
            context->frag_timeout = (u_int32_t)value;
        }
        ...
        else if(!strcasecmp(index, "min_fragment_length"))
        {
            ...
            increment = 2;
            context->min_fragment_length = (u_int32_t)value;
        }
        else if(!strcasecmp(index, "overlap_limit"))
        {
            ...
            increment = 2;
            context->overlap_limit = (u_int32_t)value;
        }
        else
        {
        ...
        Frag3Context *f3context)
{
    ...
    u_int32_t frag_end;       /* calculated end point for this fragment */
    ...
    const uint8_t *fragStart;
    int16_t fragLength;
    u_int32_t reassembled_pkt_size;
    PROFILE_VARS;
...
static void Frag3ReloadGlobal(struct _SnortConfig *sc, char *args, void **new_config)
{
    ...
    if (policy_id != getDefaultPolicy())
    {
        ...
        pCurrentPolicyConfig->static_frags = pDefaultPolicyConfig->static_frags;
             ...
             (pCurrentPolicyConfig->static_frags == 0))
    {
        pCurrentPolicyConfig->static_frags = (u_int32_t)pCurrentPolicyConfig->memcap /
            (sizeof(Frag3Frag) + sizeof(uint8_t) * pkt_snaplen) + 1;
...
static u_int32_t Frag3MemReloadAdjust(unsigned maxWork)
{
     Frag3Frag *tmp;
     SFXHASH_NODE *hnode;
#ifdef REG_TEST
     static u_int32_t addstaticfrag_cnt, delstaticfrag_cnt;
#endif
