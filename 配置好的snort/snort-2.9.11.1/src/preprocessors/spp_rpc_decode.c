...
#define RPC_FRAG_HDR_SIZE  sizeof(u_int32_t)
#define RPC_FRAG_LEN(ptr)  (ntohl(*((u_int32_t *)ptr)) & 0x7FFFFFFF)
...
static const u_int32_t flush_size = 28;
static const u_int32_t rpc_memcap = 1048510;
static u_int32_t rpc_memory = 0;
...
static RpcStatus RpcPrepRaw(const uint8_t *, u_int32_t, Packet *);
static RpcStatus RpcPrepFrag(RpcSsnData *, Packet *);
static RpcStatus RpcPrepSeg(RpcSsnData *, Packet *);
static inline u_int32_t RpcBufLen(RpcBuffer *);
static inline uint8_t * RpcBufData(RpcBuffer *);
static RpcStatus RpcBufAdd(RpcBuffer *, const uint8_t *, u_int32_t);
static inline void RpcBufClean(RpcBuffer *);

static inline void * RpcAlloc(u_int32_t);
static inline void RpcFree(void *, u_int32_t);
        ...
        RpcSsnData *rsdata, Packet *p)
{
    ...
    u_int32_t seq = ntohl(p->tcph->th_seq);
    int need;
    RpcStatus status;
...
static RpcStatus RpcPrepRaw(const uint8_t *data, u_int32_t fraglen, Packet *p)
{
    int status;
...
{
    int status;
    u_int32_t fraghdr = htonl(RpcBufLen(&rsdata->frag));
...
static RpcStatus RpcPrepSeg(RpcSsnData *rsdata, Packet *p)
{
    ...
    {
        rsdata->ignore = (sizeof(u_int32_t) + rsdata->frag_len) - RpcBufLen(&rsdata->seg);
        DEBUG_WRAP(DebugMessage(DEBUG_RPC,
                    "STATEFUL: Ignoring %u bytes\n", rsdata->ignore););
        RpcBufClean(&rsdata->seg);
    }
        ...
        RpcSsnData *rsdata, const uint8_t *fragment)
{
    ...
    u_int32_t frag_len = RPC_FRAG_LEN(fragment);
    ...
    if (RpcBufAdd(&rsdata->frag,
                fragment + sizeof(u_int32_t), frag_len) != RPC_STATUS__SUCCESS)
    {
        return RPC_STATUS__ERROR;
    }
...
static inline u_int32_t RpcBufLen(RpcBuffer *buf)
{
...
static RpcStatus RpcBufAdd(RpcBuffer *buf, const uint8_t *data, u_int32_t dsize)
{
    const u_int32_t min_alloc = flush_size;
    u_int32_t alloc_size = dsize;
    int status;
    ...
    {
        u_int32_t new_size = buf->len + alloc_size;
...
static inline void * RpcAlloc(u_int32_t size)
{
...
static inline void RpcFree(void *data, u_int32_t size)
{
    if (data == NULL)
        return;
...
static int ConvertRPC(RpcDecodeConfig *rconfig, RpcSsnData *rsdata, Packet *p)
{
    ...
    u_int32_t length;          /* length of current fragment */
    int last_fragment = 0; /* have we seen the last fragment sign? */
    u_int32_t decoded_len; /* our decoded length is always atleast a 0 byte header */
    ...
    /* cheesy alignment safe fraghdr = *(u_int32_t *) data*/
