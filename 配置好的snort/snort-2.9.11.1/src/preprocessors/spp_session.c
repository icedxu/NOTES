...
u_int32_t session_mem_in_use = 0;
...
static SessionCache* initSessionCache(u_int32_t session_type, u_int32_t protocol_scb_size, SessionCleanup clean_fcn);
...
static int initSessionKeyFromPktHeader( sfaddr_t* srcIP, uint16_t srcPort, sfaddr_t* dstIP,
        ...
        u_int32_t mplsId, uint16_t addressSpaceId, SessionKey *key);
...
static void removeSessionFromProtoOneWayList(u_int32_t proto, void *scb);
...
static int deleteSessionCache(u_int32_t protocol);
static int purgeSessionCache(SessionCache*);
static int pruneSessionCache(SessionCache*, u_int32_t thetime, void *save_me, int memcheck);
static void cleanProtocolSessionsPool( u_int32_t  );
static void freeProtocolSessionsPool( u_int32_t, void * );
static void *allocateProtocolSession( u_int32_t );
static int getActiveSessionCount(SessionCache*);
static u_int32_t getSessionPruneCount( u_int32_t );
static void resetSessionPruneCount( u_int32_t );
static void checkSessionTimeout( u_int32_t, time_t );
...
static u_int32_t getPacketDirection( Packet *p );
...
static int ignoreChannel( const Packet *ctrlPkt, sfaddr_t* srcIP, uint16_t srcPort, sfaddr_t* dstIP,
        uint16_t dstPort, uint8_t protocol, u_int32_t preprocId, char direction, char flags,
...
static int setApplicationData( void *scbptr, u_int32_t protocol, void *data, StreamAppDataFree free_func );
static void *getApplicationData( void *scbptr, u_int32_t protocol );
static StreamSessionKey * getSessionKeyFromPacket( Packet *p );
static void * getApplicationDataFromSessionKey( const StreamSessionKey *key, u_int32_t protocol);
static void *getApplicationDataFromIpPort( sfaddr_t* srcIP, uint16_t srcPort, sfaddr_t* dstIP,
        ...
        u_int32_t mplsId, uint16_t addressSpaceId, u_int32_t protocol );
static void setSessionExpirationTime(Packet *p, void *scbptr, u_int32_t timeout);
static int getSessionExpirationTime(Packet *p, void *scbptr);
static u_int32_t setSessionFlags( void *scbptr, u_int32_t flags );
static u_int32_t getSessionFlags( void *scbptr );
...
static int setAppProtocolIdExpected( const Packet *ctrlPkt, sfaddr_t* srcIP, uint16_t srcPort, sfaddr_t* dstIP,
        uint16_t dstPort, uint8_t protocol, int16_t protoId, u_int32_t preprocId,
        void* protoData, void (*protoDataFreeFn)(void*),
        struct _ExpectNode** packetExpectedNode);
#ifdef TARGET_BASED
static void registerApplicationHandler( u_int32_t preproc_id, int16_t app_id );
static int16_t getAppProtocolId( void *scbptr );
static int16_t setAppProtocolId( void *scbptr, int16_t id );
static sfaddr_t* getSessionIpAddress( void *scbptr, u_int32_t direction );
...
static void *getSessionHandleFromIpPort( sfaddr_t* srcIP, uint16_t srcPort, sfaddr_t* dstIP,
        uint16_t dstPort, char ip_protocol, uint16_t vlan,
        u_int32_t mplsId, uint16_t addressSpaceId );
...
static void disablePreprocForSession( void *scbptr, u_int32_t preproc_id );
static void enablePreprocForPort( SnortConfig *sc, u_int32_t preproc_id, u_int32_t proto, uint16_t port );
static void enablePreprocAllPorts( SnortConfig *sc, u_int32_t preproc_id, u_int32_t proto );
static void enablePreprocAllPortsAllPolicies( SnortConfig *sc, u_int32_t preproc_id, u_int32_t proto );
static bool isPreprocEnabledForPort( u_int32_t preproc_id, uint16_t port );
...
static char** getHttpXffPrecedence(void* ssn, u_int32_t flags, int* nFields);
...
void initializeMaxExpectedFlows( SessionConfiguration *sessionConfig )
{
    ...
    u_int32_t max = sessionConfig->max_tcp_sessions + sessionConfig->max_udp_sessions;
...
static int verifySessionConfig( struct _SnortConfig *sc )
{
    ...
    session_configuration->policy_ref_count = calloc( sc->num_policies_allocated, sizeof( u_int32_t ) );
...
static inline bool isPreprocEnabledForSession( SessionControlBlock *scb, u_int32_t preproc_id )
{
	return  ( scb && scb->enabled_pps & (UINT64_C(1) << preproc_id) ) ? true : false;
}

// Turn ON a preprocessor for a session
static inline void enablePreprocForSession( SessionControlBlock *scb, u_int32_t preproc_id)
{
    if( scb != NULL )
...
static u_int32_t getSessionPruneCount( u_int32_t protocol )
{
...
static void resetSessionPruneCount( u_int32_t protocol )
{
    if( protocol < SESSION_PROTO_MAX )
    {
        if( proto_session_caches[protocol] )
...
static bool prune_more_sessions( SessionCache *session_cache, u_int32_t num_pruned,
        u_int32_t prune_stop_threshold, int memCheck )
{
...
static int pruneSessionCache( SessionCache* session_cache, u_int32_t thetime, void *save_me_session, int memCheck )
{
    ...
    else
    {
         ...
         */
        u_int32_t prune_stop_threshold = session_cache->max_sessions - session_cache->cleanup_sessions;
...
static void cleanProtocolSessionsPool( u_int32_t protocol )
{
    if( protocol < SESSION_PROTO_MAX )
    {
        if( proto_session_caches[protocol] )
...
static void freeProtocolSessionsPool( u_int32_t protocol, void *scb )
{
    if( protocol < SESSION_PROTO_MAX )
    {
        if( proto_session_caches[protocol] )
...
static void *allocateProtocolSession( u_int32_t protocol )
{
    if( protocol < SESSION_PROTO_MAX )
    {
        if( proto_session_caches[protocol] )
...
static u_int32_t HashFunc(SFHASHFCN *p, unsigned char *d, int n)
{
    u_int32_t a,b,c;
    u_int32_t offset = 0;
#ifdef MPLS
    ...
    offset=36;
    a += *(u_int32_t*)(d+offset);   /* vlan, protocol, & pad */
#ifdef MPLS
...
#endif
#ifdef HAVE_DAQ_ADDRESS_SPACE_ID
    tmp2 = *(u_int32_t*)(d+offset); /* after offset that has been moved */
...
static SessionCache* initSessionCache(u_int32_t session_type, u_int32_t protocol_scb_size, SessionCleanup cleanup_fcn)
{
    int hashTableSize;
    SessionCache *sessionCache = NULL;
    u_int32_t max_sessions = 0, session_timeout_min = 0, session_timeout_max = 0;
    u_int32_t cleanup_sessions = 5;
...
static void checkCacheFlowTimeout(u_int32_t flowCount, time_t cur_time, SessionCache *cache)
{
    u_int32_t flowRetiredCount = 0, flowExaminedCount = 0;
    SessionControlBlock *scb;
...
static void checkSessionTimeout(u_int32_t flowCount, time_t cur_time)
{
    Active_Suspend( );
...
static void setSessionExpirationTime(Packet *p, void *scb, u_int32_t timeout)
{
...
static int ignoreChannel( const Packet *ctrlPkt, sfaddr_t* srcIP, uint16_t srcPort,
        sfaddr_t* dstIP, uint16_t dstPort, uint8_t protocol, u_int32_t preprocId,
...
static int setAppProtocolIdExpected( const Packet *ctrlPkt, sfaddr_t* srcIP, uint16_t srcPort,
        sfaddr_t* dstIP, uint16_t dstPort, uint8_t protocol, int16_t protoId, u_int32_t preprocId,
...
void SessionReload(SessionCache* lws_cache, u_int32_t max_sessions,
                   u_int32_t aggressiveTimeout, u_int32_t nominalTimeout
#ifdef REG_TEST
                   , const char* name
#endif
                   )
...
unsigned SessionProtocolReloadAdjust(SessionCache* lws_cache, u_int32_t max_sessions,
                                     unsigned maxWork, u_int32_t memcap
#ifdef REG_TEST
                                     , const char* name
#endif
                                     )
...
#ifdef ENABLE_HA
static bool verifyConfigOptionUnchanged( u_int32_t new, u_int32_t old, char *name, SessionConfiguration *config )
{
    if( old == new )
        return false;
...
static int verifyReloadedSessionConfiguration( struct _SnortConfig *sc, void *swap_config )
{
    ...
    ssc->policy_ref_count = calloc( sc->num_policies_allocated, sizeof( u_int32_t ) );
