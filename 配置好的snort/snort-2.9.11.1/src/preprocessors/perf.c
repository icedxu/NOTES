...
static inline bool sfCheckFileSize(FILE *, u_int32_t);
...
static int sfRotateFile(const char *, FILE *, const char *, u_int32_t);
...
static int sfRotateFile(const char *old_file, FILE *old_fh,
        const char *rotate_prefix, u_int32_t max_file_size)
{
    time_t ts;
    struct tm *tm;
        ...
        do
        {
            ...
            while (!feof(old_fh))
            {
                ...
                if (num_read > 0)
                {
                    ...
                    if (((u_int32_t)file_stats.st_size + num_read) > max_file_size)
                    {
                        fclose(rotate_fh);
...
static inline bool sfCheckFileSize(FILE *fh, u_int32_t max_file_size)
{
    int fd;
    struct stat file_stats;
    ...
    if ((fstat(fd, &file_stats) == 0)
            && ((u_int32_t)file_stats.st_size >= max_file_size))
        return true;
