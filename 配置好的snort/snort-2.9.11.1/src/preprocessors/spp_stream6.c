...
u_int32_t xtradata_func_count = 0;
...
static int StreamAddSessionAlert(void *ssnptr, Packet *p, u_int32_t gid, u_int32_t sid);
static int StreamCheckSessionAlert(void *ssnptr, Packet *p, u_int32_t gid, u_int32_t sid);
static int StreamUpdateSessionAlert(void *ssnptr, Packet *p, u_int32_t gid, u_int32_t sid,
...
static u_int32_t StreamGetFlushPoint(void *ssnptr, char dir);
static void StreamSetFlushPoint(void *ssnptr, char dir, u_int32_t flush_point);
...
static u_int32_t StreamRegisterXtraData(LogFunction );
static u_int32_t StreamGetXtraDataMap(LogFunction **);
static void StreamRegisterXtraDataLog(LogExtraData, void * );
static void StreamSetExtraData(void* ssn, Packet*, u_int32_t);
static void StreamClearExtraData(void* ssn, Packet*, u_int32_t);
...
static u_int32_t StreamGetPreprocFlags( void *ssnptr);
...
static int StreamSetApplicationProtocolIdExpectedPreassignCallbackId( const Packet *ctrlPkt, sfaddr_t* srcIP,
        ...
        u_int32_t preprocId, void *protoData, void (*protoDataFreeFn)(void*), unsigned cbId, Stream_Event se,
...
static void checkOnewayStatus( u_int32_t protocol, SessionControlBlock *scb )
{
...
{
    uint8_t layerIndex;
    u_int32_t direction = session_api->get_packet_direction(p);
...
static void StreamSetExtraData (void* pv, Packet* p, u_int32_t flag)
{
...
static void StreamClearExtraData (void* pv, Packet* p, u_int32_t flag)
{
    SessionControlBlock* ssn = pv;
...
static u_int32_t StreamRegisterXtraData( LogFunction f )
{
    u_int32_t i = 0;
...
static u_int32_t StreamGetXtraDataMap( LogFunction **f )
{
    if( f )
    {
        *f = xtradata_map;
...
static int StreamSetApplicationProtocolIdExpectedPreassignCallbackId( const Packet *ctrlPkt,
        ...
        uint8_t protocol, int16_t protoId, u_int32_t preprocId, void *protoData,
