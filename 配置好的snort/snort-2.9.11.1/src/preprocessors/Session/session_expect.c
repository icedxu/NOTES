...
int StreamExpectAddChannel(const Packet *ctrlPkt, sfaddr_t* cliIP, uint16_t cliPort,
        ...
        uint8_t protocol, u_int32_t timeout, int16_t appId, u_int32_t preprocId,
        ...
        Stream_Event se, ExpectNode** packetExpectedNode)
{
     ...
     */
    hashKey.protocol = (u_int32_t)protocol;
...
void* getApplicationDataFromExpectedNode(ExpectNode* expectedNode, u_int32_t preprocId)
{
    ExpectedSessionData *data;
...
int addApplicationDataToExpectedNode(ExpectNode* expectedNode, u_int32_t preprocId,
...
int StreamExpectIsExpected(Packet *p, SFXHASH_NODE **expected_hash_node)
{
    ...
    hashKey.protocol = (u_int32_t)GET_IPH_PROTO(p);
...
void StreamExpectInit (u_int32_t max)
{
    if (!channelHash)
    {
