...
// config stuff
static u_int32_t hi_cap = 0;

// stats
static u_int32_t hi_paf_calls = 0;
static u_int32_t hi_paf_bytes = 0;
...
static void hi_update_flow_depth_state(Hi5State *hip, u_int32_t* fp, PAF_Status *paf );
...
{
    u_int32_t nreq = s_req->pipe & 0xFF;
    u_int32_t pipe = s_req->pipe >> 8;
...
static inline PAF_Status hi_scan_msg (
    Hi5State* s, int c, u_int32_t* fp, void* ssn)
{
    PAF_Status paf = PAF_SEARCH;
    DEBUG_WRAP(DebugMessage(DEBUG_STREAM_PAF,
...
static PAF_Status hi_paf (
    void* ssn, void** pv, const uint8_t* data, u_int32_t len,
    uint64_t *flags, u_int32_t* fp, u_int32_t *fp_eoh)
{
    Hi5State* hip = *pv;
    PAF_Status paf = PAF_SEARCH;
...
bool hi_paf_init (u_int32_t cap)
{
    DEBUG_WRAP(DebugMessage(DEBUG_STREAM_PAF,
        "%s: cap=%u\n",  __FUNCTION__, cap);)
...
u_int32_t hi_paf_resp_bytes_processed(void* ssn)
{
    if ( ssn )
    {
        Hi5State** s = (Hi5State **)stream_api->get_paf_user_data(ssn, 0, hi_paf_id);
