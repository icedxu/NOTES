...
int CheckChunkEncoding(HI_SESSION *Session, const u_char *start, const u_char *end,
        const u_char **post_end, u_char *iChunkBuf, u_int32_t max_size,
        u_int32_t chunk_remainder, u_int32_t *updated_chunk_remainder, u_int32_t *chunkRead, HttpSessionData *hsd,
        int iInspectMode)
{
    u_int32_t iChunkLen   = 0;
    u_int32_t iChunkChars = 0;
    int chunkPresent = 0;
    u_int32_t iCheckChunk = 1;
    const u_char *ptr;
    const u_char *jump_ptr;
    u_int32_t iDataLen = 0;
    u_int32_t chunkBytesCopied = 0;
        ...
        const u_char *end, HEADER_PTR *header_ptr, HEADER_FIELD_PTR *header_field_ptr, int iInspectMode)
{
         ...
         header_field_ptr->content_len->cont_len_end )
    {
            ...
            || (len > 0xFFFFFFFF) )
        {
            ...
            header_field_ptr->content_len->len = (u_int32_t)len;
    }
