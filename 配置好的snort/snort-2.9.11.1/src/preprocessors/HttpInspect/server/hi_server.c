...
extern int CheckChunkEncoding(HI_SESSION *, const u_char *, const u_char *, const u_char **, u_char *,
                              u_int32_t , u_int32_t, u_int32_t *, u_int32_t *, HttpSessionData *, int);
                        ...
                        const u_char *ptr, const u_char *end, URI_PTR *result)
{
    ...
    u_int32_t updated_chunk_remainder = 0;
    u_int32_t chunk_read = 0;
        ...
        const u_char *end, URI_PTR *result)
{
    ...
    static u_int32_t updated_chunk_remainder = 0;
    static u_int32_t chunk_read = 0;
    u_int32_t saved_chunk_size = 0;
...
void ApplyFlowDepth(HTTPINSPECT_CONF *ServerConf, Packet *p,
        HttpSessionData *sd, int resp_header_size, int read_only, u_int32_t seq_num)
{
        ...
        if(sd != NULL)
        {
            if(!(sd->resp_state.flow_depth_excd ))
            {
                if(sd->resp_state.max_seq)
                {
                    ...
                    {
                        if(((u_int32_t)p->dsize) > (sd->resp_state.max_seq- seq_num))
                        {
        ...
        int dsize, HttpSessionData *sd)
{
    ...
    static u_int32_t paf_bytes_total = 0;
    static int paf_bytes_curr = 0;
    u_int32_t paf_bytes_processed = 0;
                    ...
                    if (sd != NULL)
                    {
                        ...
                        if (p->packet_flags & PKT_STREAM_INSERT)
                        {
                            if ( ScPafEnabled() )
                            {
                                ...
                                ((u_int32_t)(end - (header_ptr.header.uri_end)) >= header_ptr.content_len.len))
                            {
