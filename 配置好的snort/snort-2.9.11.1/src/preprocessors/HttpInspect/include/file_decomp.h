...
struct fd_session_s
{
    ...
    u_int32_t Avail_In;  /* number of bytes available at next_in */
    u_int32_t Total_In;  /* total number of input bytes read so far */

    uint8_t *Next_Out;  /* next output byte should be put there */
    u_int32_t Avail_Out; /* remaining free space at next_out */
    u_int32_t Total_Out; /* total number of bytes output so far */

    /* Internal buffer setup by _Init().  App can overide. */
    uint8_t *Buffer;    /* pointer to decompresiion buffer */
    u_int32_t Buffer_Len;/* length of decompression buffer */

    /* Configuration settings */
    u_int32_t Compr_Depth;
    u_int32_t Decompr_Depth;
    u_int32_t Modes;     /* Bit mapped set of potential file/algo modes */
