                                   ...
                                   char *ErrorString, int ErrStrLen)
{
    ...
    while ((pcToken = strtok(NULL, CONF_SEPARATORS)) != NULL)
    {
        ...
        if (num_toks == 1)
        {
            u_int32_t chunk_length = (u_int32_t)SnortStrtoulRange(pcToken, &pcEnd, 10, 0, UINT8_MAX);
            if ((pcEnd == pcToken) || *pcEnd || (errno == ERANGE))
            {
                SnortSnprintf(ErrorString, ErrStrLen,
        ...
        {
            u_int32_t num_chunks_threshold = (u_int32_t)SnortStrtoulRange(pcToken, &pcEnd, 10, 0, UINT8_MAX);
            if ((pcEnd == pcToken) || *pcEnd || (errno == ERANGE))
            {
                SnortSnprintf(ErrorString, ErrStrLen,
...
int GetHttpTrueIP(void *data, uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
    sfaddr_t *true_ip;
...
int GetHttpGzipData(void *data, uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
    if(!IsGzipData(data))
    {
...
int GetHttpJSNormData(void *data,  uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
...
int GetHttpUriData(void *data, uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
    HttpSessionData *hsd = NULL;
...
int GetHttpHostnameData(void *data, uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
    HttpSessionData *hsd = NULL;
...
static int GetHttpInspectConf( void *ssn, u_int32_t flags, HTTPINSPECT_CONF **serverConf, HTTPINSPECT_CONF **clientConf )
{
    int iRet = HI_SUCCESS;
...
int GetHttpFlowDepth(void *ssn, u_int32_t flags)
{
