...
static u_int32_t pkt_snaplen = 0;
...
typedef struct _StreamTracker
{
    ...
    u_int32_t flush_count;        /* number of flushed queued segments */
    u_int32_t xtradata_mask;      /* extra data available to log */
...
static inline u_int32_t SegsToFlush (const StreamTracker* st, unsigned max)
{
    u_int32_t n = st->seg_count - st->flush_count;
    StreamSegment* s;
...
static inline void QueueState(uint8_t, StreamTracker*, uint8_t,
        u_int32_t, uint8_t);
static inline int EvalStateQueue(StreamTracker *, uint8_t, u_int32_t);
#endif
...
static int StreamSeglistDeleteNodeTrim(StreamTracker*, StreamSegment*, u_int32_t flush_seq);
...
static u_int32_t StreamGetWscale(Packet *, uint16_t *);
static u_int32_t StreamPacketHasWscale(Packet *);
static u_int32_t StreamGetMss(Packet *, uint16_t *);
static u_int32_t StreamGetTcpTimestamp(Packet *, u_int32_t *, int strip);
static int FlushStream(
        Packet*, StreamTracker *st, u_int32_t toSeq, uint8_t *flushbuf,
...
static inline void InitFlushPointList(FlushPointList *flush_point_list, u_int32_t value, u_int32_t range, int use_static)
{
    u_int32_t i;
    ...
    if (!flush_point_list->initialized)
    {
        ...
        u_int32_t cfp = sfp ? atoi(sfp) : 0;
        if ( cfp < 128 || cfp > 255 ) cfp = 192;
#else
        const u_int32_t cfp = 192;
#endif
...
static inline void UpdateFlushMgr(
        FlushMgr *mgr, FlushPointList *flush_point_list, u_int32_t flags)
{
...
u_int32_t StreamGetPreprocFlagsTcp(SessionControlBlock *scb)
{
    TcpSession *tcpssn;
...
static void addStreamTcpPolicyToList( StreamTcpConfig *config, StreamTcpPolicy *policy )
{
    ...
    {
        u_int32_t policyListSize = sizeof( StreamTcpPolicy * ) * ( config->num_policies );
...
u_int32_t StreamGetTcpPrunes(void)
{
    if( tcp_lws_cache )
...
static inline void EventInternal (u_int32_t eventSid)
{
...
static inline void NormalTrimPayload (Packet* p, u_int32_t max, TcpDataBlock* tdb)
{
...
static inline bool NormalTrimPayloadIfSyn ( Packet *p, u_int32_t max, TcpDataBlock *tdb )
{
...
static inline bool NormalTrimPayloadIfRst ( Packet *p, u_int32_t max, TcpDataBlock *tdb )
{
...
static inline bool NormalTrimPayloadIfWin ( Packet *p, u_int32_t max, TcpDataBlock *tdb )
{
...
static inline bool NormalTrimPayloadIfMss ( Packet *p, u_int32_t max, TcpDataBlock *tdb )
{
...
static inline void GetPacketHeaderFoo (
        ...
        DAQ_PktHdr_t* pkth, u_int32_t dir)
{
...
static inline u_int32_t StreamGetWindow(
...
{
    u_int32_t win = StreamGetWindow(scb, st, tdb);
...
static inline void UpdateSsn(Packet* p, StreamTracker *rcv, StreamTracker *snd, TcpDataBlock *tdb)
{
    ...
    if (
        ...
        rcv->s_mgr.state == TCP_STATE_ESTABLISHED &&
            ...
            while ( seg )
            {
            u_int32_t seq = seg->seq + seg->size;
...
static inline int purge_alerts(StreamTracker *st, u_int32_t flush_seq, void *ssnptr)
{
    int i;
    int new_count = 0;
...
static inline int purge_flushed_ackd (TcpSession *tcpssn, StreamTracker *st)
{
    ...
    {
        u_int32_t end = seg->seq + seg->size;
...
static inline int _flush_to_seq_noack( TcpSession *tcpssn, StreamTracker *st, u_int32_t bytes,
        Packet *p, sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, u_int32_t dir )
{
    u_int32_t stop_seq;
...
static inline int _flush_to_seq (
        ...
        TcpSession *tcpssn, StreamTracker *st, u_int32_t bytes, Packet *p,
        sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, u_int32_t dir)
{
    u_int32_t start_seq;
    u_int32_t stop_seq;
    u_int32_t footprint = 0;
...
static inline int flush_to_seq_noack( TcpSession *tcpssn, StreamTracker *st, u_int32_t bytes,
        Packet *p, sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, u_int32_t dir )
{
    STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE,
    ...
    {
        u_int32_t missed = st->seglist_next->seq - st->seglist_base_seq;
...
static inline u_int32_t get_q_sequenced(StreamTracker *st)
{
    u_int32_t len;
...
static inline int flush_ackd(
        ...
        sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, u_int32_t dir)
{
    u_int32_t bytes = get_q_footprint(st);
        ...
        sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, u_int32_t dir)
{
    ...
    {
        u_int32_t bytes = get_q_sequenced(st);
        return flush_to_seq(tcpssn, st, bytes, p, sip, dip, sp, dp, dir);
    }
#endif
    fm = &tcpssn->server.flush_mgr;
...
static int FlushStream(
        Packet* p, StreamTracker *st, u_int32_t toSeq, uint8_t *flushbuf,
        const uint8_t *flushbuf_end)
{
        ...
        if (tcpssn->client.seglist && !(scb->ha_state.ignore_direction & SSN_DIR_FROM_SERVER) )
        {
            ...
            //set policy id for this packet
            {
                ...
                if (srcIp && dstIp)
                {
                    ...
                    {
                        memcpy(&((((DAQ_PktHdr_t*)p->pkth)->real_sIP).s6_addr), &(srcIp)->ia32[3], sizeof(u_int32_t));
                        memcpy(&((((DAQ_PktHdr_t*)p->pkth)->real_dIP).s6_addr), &(dstIp)->ia32[3], sizeof(u_int32_t));
                    }
...
static void TraceEvent (
        const Packet* p, TcpDataBlock* tdb, u_int32_t txd, u_int32_t rxd
        ) {
    int i;
    char flags[7] = "UAPRSF";
...
{
    u_int32_t why = a->l_nxt_seq ? LCL(a, l_nxt_seq) : 0;
...
static u_int32_t StreamGetTcpTimestamp( Packet *p, u_int32_t *ts, int strip )
{
    unsigned int i = 0;
...
static u_int32_t StreamGetMss(Packet *p, uint16_t *value)
{
    unsigned int i = 0;
...
static u_int32_t StreamGetWscale(Packet *p, uint16_t *value)
{
    unsigned int i = 0;
...
static u_int32_t StreamPacketHasWscale(Packet *p)
{
    uint16_t wscale;
...
static inline void QueueState(uint8_t transition, StreamTracker *st,
        uint8_t expected_flags, u_int32_t seq_num, uint8_t get_seq)
{
...
static inline StreamSegment* SegmentAlloc (
        Packet* p, const struct timeval* tv, u_int32_t caplen, u_int32_t pktlen, const uint8_t* pkt)
{
    StreamSegment* ss;
    ...
    if(st->s_mgr.state_queue == TCP_STATE_CLOSE_WAIT)
    {
        ...
        switch(reassembly_policy)
        {
            ...
            default:
                ...
                {
                    u_int32_t delta = seq + newSize - (st->s_mgr.transition_seq - 1);
...
static inline bool IsRetransmit(StreamSegment *seg, const uint8_t *rdata,
        uint16_t rsize, u_int32_t rseq, bool check_frt, bool *full_retransmit)
{
        ...
        StreamTcpPolicy *s5TcpPolicy)
{
    ...
    if(rcv->seg_count != 0)
    {
        ...
        else
        {
            ...
            {
                u_int32_t offset = rcv->r_win_base - tdb->seq;
        ...
        TcpDataBlock *tdb, StreamTcpPolicy *s5TcpPolicy)
{
    ...
    if((tdb->seq == listener->r_nxt_ack) || (p->tcph->th_flags & TH_SYN))
    {
                ...
                (Stream_NormGetMode(listener->reassembly_policy, snort_conf, NORM_TCP_IPS) == NORM_MODE_ON))
        {
            ...
            {
                u_int32_t delta = tdb->end_seq - (listener->s_mgr.transition_seq - 1);
                if (p->dsize > delta)
                      ...
                      StreamTcpPolicy *s5TcpPolicy, SFXHASH_NODE *hash_node)
{
    ...
    StreamTracker *talker = NULL;
    StreamTracker *listener = NULL;
    u_int32_t require3Way = (s5TcpPolicy->flags & STREAM_CONFIG_REQUIRE_3WHS);
        ...
        else
        {
                ...
                (listener->flush_mgr.flush_policy == STREAM_FLPOLICY_IGNORE))
            {
                ...
                if (!process_fin && ((talker->s_mgr.state == TCP_STATE_FIN_WAIT_1) ||
                        (talker->s_mgr.state == TCP_STATE_LAST_ACK)))
                {
                    u_int32_t end_seq = ( scb->ha_state.session_flags & SSNFLAG_MIDSTREAM ) ?
...
dupfin:
            ...
            (!TwoWayTraffic(scb)&& (talker->s_mgr.state == TCP_STATE_CLOSED || listener->s_mgr.state == TCP_STATE_CLOSED)))
    {
        ...
        if(p->packet_flags & PKT_FROM_SERVER)
        {
            ...
            {
                u_int32_t flushed = flush_stream(tcpssn, talker, p,
                        GET_DST_IP(p), GET_SRC_IP(p),
                        p->tcph->th_dport, p->tcph->th_sport,
                        PKT_FROM_CLIENT);
            ...
            {
                u_int32_t flushed = flush_stream(tcpssn, listener, p,
                        GET_SRC_IP(p), GET_DST_IP(p),
                        p->tcph->th_sport, p->tcph->th_dport,
                        PKT_FROM_SERVER);
...
static inline u_int32_t GetReverseDir (const Packet* p)
{
...
#ifdef NORMALIZER
static inline u_int32_t GetForwardDir (const Packet* p)
{
    if ( p->packet_flags & PKT_FROM_SERVER )
        return PKT_FROM_SERVER;
                                       ...
                                       StreamTracker *trk, Packet *pkt, uint64_t *flags )
{
    ...
    {
        u_int32_t flush_pt;
        u_int32_t size = seg->size;
        u_int32_t end = seg->seq + seg->size;
        u_int32_t pos = s5_paf_position(&trk->paf_state);
                ...
                if ( Normalize_GetMode(snort_conf, NORM_TCP_IPS) == NORM_MODE_ON )
                {
                    ...
                    if(!(tcpssn->pp_flags & PP_FTPTELNET_FTPS))
                    {
                        if(!listener->flush_mgr.last_size)
                        {
                            ...
                            {
                                u_int32_t ftp_data_conn_mss = (tcpssn->client.mss < tcpssn->server.mss) ? tcpssn->client.mss : tcpssn->server.mss;
        ...
        case STREAM_FLPOLICY_PROTOCOL_NOACK:
            {
                ...
                u_int32_t flush_amt = flush_pdu_ips( config, tcpssn, listener, p, &flags);
                u_int32_t this_flush;
...
static inline u_int32_t flush_pdu_ackd ( StreamTcpConfig *config, TcpSession* ssn,
                                        StreamTracker* trk, Packet* pkt, uint64_t* flags)
{
    bool to_srv = ( *flags == PKT_FROM_CLIENT );
        ...
        case STREAM_FLPOLICY_PROTOCOL:
            {
                ...
                u_int32_t flush_amt = flush_pdu_ackd( config, tcpssn, talker, p, &flags);
...
static int StreamSeglistDeleteNodeTrim (
        StreamTracker* st, StreamSegment* seg, u_int32_t flush_seq)
{
    assert(st && seg);
    ...
    {
        u_int32_t delta = flush_seq - seg->seq;
...
int GetTcpRebuiltPackets(Packet *p, SessionControlBlock *ssn, PacketIterator callback, void *userdata)
{
    ...
    StreamTracker *st;
    StreamSegment *ss;
    u_int32_t start_seq = ntohl(p->tcph->th_seq);
    u_int32_t end_seq = start_seq + p->dsize;
        ...
        void *userdata)
{
    ...
    for (; ss && ((ss->buffered == SL_BUF_FLUSHED) || (tcpssn->pp_flags & PP_HTTPINSPECT_PAF_FLUSH_POST_HDR)); ss = ss->next)
    {
        ...
        {
            DAQ_PktHdr_t pkth;
            u_int32_t ajust_seq = ss->seq - (u_int32_t)(ss->payload - ss->data);
            pkth.ts.tv_sec = ss->tv.tv_sec;
            pkth.ts.tv_usec = ss->tv.tv_usec;
            pkth.caplen = ss->caplen;
            pkth.pktlen = ss->pktlen;
...
int StreamAddSessionAlertTcp( SessionControlBlock* scb, Packet* p, u_int32_t gid, u_int32_t sid)
{
    TcpSession *tcpssn = NULL;
    StreamTracker *st;
    StreamAlertInfo* ai;
...
int StreamCheckSessionAlertTcp(SessionControlBlock *scb, Packet *p, u_int32_t gid, u_int32_t sid)
{
    TcpSession *tcpssn = NULL;
    StreamTracker *st;
    int i;
...
int StreamUpdateSessionAlertTcp(SessionControlBlock *scb, Packet *p, u_int32_t gid, u_int32_t sid,
...
void StreamSetExtraDataTcp (SessionControlBlock* scb, Packet* p, u_int32_t xid)
{
    TcpSession *tcpssn = NULL;
    StreamTracker *st;
...
void StreamClearExtraDataTcp (SessionControlBlock* scb, Packet* p, u_int32_t xid)
{
    TcpSession *tcpssn = NULL;
    StreamTracker *st;
...
u_int32_t StreamGetFlushPointTcp(SessionControlBlock *scb, char dir)
{
    TcpSession *tcpssn = NULL;
...
void StreamSetFlushPointTcp(SessionControlBlock *scb, char dir, u_int32_t flush_point)
{
    TcpSession *tcpssn = NULL;
...
#ifdef SNORT_RELOAD
void SessionTCPReload(u_int32_t max_sessions, uint16_t pruningTimeout, uint16_t nominalTimeout)
{
