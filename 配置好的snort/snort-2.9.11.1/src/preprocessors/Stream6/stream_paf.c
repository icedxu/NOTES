...
typedef struct {
    ...
    u_int32_t prep_calls;
    u_int32_t prep_bytes;
...
static u_int32_t s5_len;  // total bytes queued
static u_int32_t s5_idx;  // offset from start of queued bytes

//--------------------------------------------------------------------

static u_int32_t s5_paf_flush (
...
static bool s5_paf_callback (
    ...
    const uint8_t* data, u_int32_t len, uint64_t *flags)
{
...
static inline bool s5_paf_eval (
    ...
    uint16_t port, uint64_t *flags, u_int32_t fuzz,
    const uint8_t* data, u_int32_t len, FlushType* ft)
{
    DEBUG_WRAP(DebugMessage(DEBUG_STREAM_PAF,
...
u_int32_t s5_paf_check (
    void* pv, PAF_State* ps, void* ssn,
    const uint8_t* data, u_int32_t len, u_int32_t total,
    u_int32_t seq, uint16_t port, uint64_t* flags, u_int32_t fuzz)
{
    PAF_Config* pc = pv;
