...
typedef struct {
    ...
    u_int32_t seq;    // stream cursor
    u_int32_t pos;    // last flush position

    u_int32_t fpt;    // current flush point
    u_int32_t tot;    // total bytes flushed

    PAF_Status paf;  // current scan state
    uint8_t mode;
    uint16_t cb_mask; // callback mask
    uint8_t cb_id[MAX_PAF_USER];
    u_int32_t fpt_eoh; //Flush point at EOH
...
static inline u_int32_t s5_paf_position (PAF_State* ps)
{
    return ps->seq;
}

static inline u_int32_t s5_paf_initialized (PAF_State* ps)
{
    return ( ps->paf != PAF_START );
}

static inline u_int32_t s5_paf_active (PAF_State* ps)
{
