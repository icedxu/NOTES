                        ...
                        OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->sigInfo.generator = (u_int32_t)gid;
    otn->event_data.sig_generator = (u_int32_t)gid;
}
                             ...
                             OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    for (i = 0; i < num_metadata_toks; i++)
    {
        ...
        else if (strcasecmp(key, METADATA_KEY__SOID) == 0)
        {
            ...
            otn->sigInfo.otnKey.gid = (u_int32_t)long_val;

            long_val = SnortStrtoul(toks[1], &endptr, 10);
            if ((errno == ERANGE) || (*endptr != '\0') || (long_val > UINT32_MAX))
                ParseError("Bogus sid %s", toks[1]);

            otn->sigInfo.otnKey.sid = (u_int32_t)long_val;
                             ...
                             OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->sigInfo.priority = (u_int32_t)priority;
    /* deprecated */
    otn->event_data.priority = (u_int32_t)priority;
}
                             ...
                             OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->sigInfo.rev = (u_int32_t)rev;
    /* deprecated */
    otn->event_data.sig_rev = (u_int32_t)rev;
}
                        ...
                        OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->sigInfo.id = (u_int32_t)sid;
    /* deprecated */
    otn->event_data.sig_id = (u_int32_t)sid;
}
