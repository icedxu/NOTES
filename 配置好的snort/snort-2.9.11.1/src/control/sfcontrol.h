...
typedef struct _CS_MESSAGE_HEADER
{
    ...
    uint16_t version;
    uint16_t type;
    u_int32_t length;    /* Does not include the header */
} CSMessageHeader;

struct _THREAD_ELEMENT;
typedef int (*ControlDataSendFunc)(struct _THREAD_ELEMENT *te, const uint8_t *data, uint16_t length);
typedef int (*OOBPreControlFunc)(uint16_t type, const uint8_t *data, u_int32_t length, void **new_context, char *statusBuf, int statusBuf_len);
