...
{
    u_int32_t encode_bytes_read;
    u_int32_t decode_bytes_read;
    int encode_depth;
    int decode_depth;
} Base64_DecodeState;
...
typedef struct s_Email_DecodeState
{
    ...
    u_int32_t prev_encoded_bytes;
    unsigned char *prev_encoded_buf;
    u_int32_t decoded_bytes;
    uint8_t *encodeBuf;
    uint8_t *decodeBuf;
    uint8_t *decodePtr;
...
// end :: start + length
int EmailDecode(const uint8_t *start, const uint8_t *end, Email_DecodeState *, uint8_t *fname, u_int32_t *fname_size, bool filename_present);
