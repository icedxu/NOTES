...
/* Version is the first four bits of the u_int32_t passed in */
#define IP6_VER(x) \
   (ntohl(x) >> 28)
...
u_int32_t ip6_ret_id(const Packet *p)
{
    IP6Frag *frag_hdr;
...
u_int32_t orig_ip6_ret_id(const Packet *p)
{
...
u_int32_t ip4_ret_id(const Packet *p)
{
    VALIDATE(p,1);
...
u_int32_t orig_ip4_ret_id(const Packet *p)
{
    VALIDATE(p,1);
