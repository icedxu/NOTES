...
static TABLE_PTR _sub_table_flat_new(dir_table_flat_t *root, u_int32_t dimension,
...
TABLE_PTR sfrt_dir_flat_new(u_int32_t mem_cap, int count,...)
{
    va_list ap;
    u_int32_t val;
    int index;
...
static void _sub_table_flat_free(u_int32_t *allocated, SUB_TABLE_PTR sub_ptr)
{
    int index;
    dir_sub_table_flat_t *sub;
    uint8_t *base;
...
static inline void _dir_fill_all(u_int32_t *allocated, u_int32_t index, u_int32_t fill,
        word length, u_int32_t val, SUB_TABLE_PTR sub_ptr)
{
...
static inline int64_t _dir_update_info(int index, int fill,
        word length, u_int32_t val, SUB_TABLE_PTR sub_ptr, updateEntryInfoFunc updateEntry, INFO *data)
{
        ...
        SUB_TABLE_PTR sub_ptr, dir_table_flat_t *root_table,updateEntryInfoFunc updateEntry, INFO *data)
{
    ...
    if(sub_table->width >= cur_len)
    {
        ...
        else if (behavior == RT_FAVOR_ALL)
        {
            ...
            root_table->allocated += (u_int32_t)bytesAllocated;
 ...
 * @param ptr       Information to be associated with this IP range
 * @param master_table    The table that describes all, returned by dir_new */
int sfrt_dir_flat_insert(u_int32_t* adr, int numAdrDwords, int len, word data_index,
...
tuple_flat_t sfrt_dir_flat_lookup(u_int32_t* adr, int numAdrDwords, TABLE_PTR table_ptr)
{
    dir_table_flat_t *root;
    uint8_t *base = (uint8_t *)segment_basePtr();
    u_int32_t h_adr[4];
...
u_int32_t sfrt_dir_flat_usage(TABLE_PTR table_ptr)
{
    dir_table_flat_t *table;
    uint8_t *base;
    if(!table_ptr)
