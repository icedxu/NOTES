...
/*pengtian*/
#if defined(WIN32) && !defined(__cplusplus) && !defined(inline)
...
static inline int sfip_fast_cont4(const sfcidr_t *ip1, const sfaddr_t *ip2) {
    u_int32_t shift = 128 - sfip_bits(ip1);
    u_int32_t ip = ntohl(sfaddr_get_ip4_value(ip2));
    u_int32_t ip3 = ntohl(sfip_get_ip4_value(ip1));
