...
table_flat_t *sfrt_flat_new(char table_flat_type, char ip_type,  long data_size, u_int32_t mem_cap)
{
    table_flat_t *table;
        ...
        int behavior, table_flat_t* table, updateEntryInfoFunc updateEntry)
{
    ...
    table->allocated += (u_int32_t)bytesAllocated;
...
u_int32_t sfrt_flat_num_entries(table_flat_t* table)
{
    if(!table)
    {
        return 0;
...
u_int32_t sfrt_flat_usage(table_flat_t *table)
{
    u_int32_t usage;
