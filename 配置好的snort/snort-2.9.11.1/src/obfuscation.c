...
static int TraverseReassembled(
        Packet *,
        int (*)(DAQ_PktHdr_t *, uint8_t *, uint8_t *, u_int32_t, void *),
        void *
        );
#endif
...
static inline void SetObfuscationEntry(ObfuscationEntry *, Packet *,
        ob_size_t, ob_size_t, ob_char_t, u_int32_t);
...
static ObRet TraverseObfuscationList(ObfuscationCallbackData *,
        const DAQ_PktHdr_t *, const uint8_t *, ob_size_t, u_int32_t);
static int ObfuscateStreamSegmentsCallback(DAQ_PktHdr_t *,
        uint8_t *, uint8_t *, u_int32_t, void *);
...
static inline u_int32_t get_pkt_seq_num(const Packet* p)
{
    u_int32_t pkt_seq = 0;
...
static inline int32_t get_length_from_seq(u_int32_t start, u_int32_t end)
{
    u_int32_t length = end - start;
...
static inline void SetObfuscationEntry(ObfuscationEntry *entry,
        Packet *p, ob_size_t offset, ob_size_t length, ob_char_t ob_char, u_int32_t seq)
{
    if (entry == NULL)
        return;
        ...
        ob_size_t length, ob_char_t ob_char)
{
    ...
    u_int32_t seq = offset + get_pkt_seq_num(p);
...
static ObRet TraverseObfuscationList(ObfuscationCallbackData *data,
        ...
        ob_size_t payload_size, u_int32_t seq_num)
{
    int i;
    ob_size_t payload_offset = 0;
    const DAQ_PktHdr_t *pkth_tmp = pkth;
    u_int32_t data_seq = get_pkt_seq_num(data->packet);
    int32_t length;
...
static int ObfuscateStreamSegmentsCallback(DAQ_PktHdr_t *pkth,
        uint8_t *pkt, uint8_t *payload, u_int32_t seq_num, void *data)
{
