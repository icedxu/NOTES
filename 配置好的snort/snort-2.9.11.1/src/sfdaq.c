...
static u_int32_t snap = PKT_SNAPLEN;
...
{
    u_int32_t have = daq_get_type(daq_mod);
    u_int32_t need = 0;
...
{
    u_int32_t caps = daq_get_capabilities(daq_mod, daq_hand);
...
static int DAQ_PreControl(uint16_t type, const uint8_t *data, u_int32_t length, void **new_config, char *statusBuf, int statusBuf_len)
{
    if (daq_mod && daq_hand)
...
int DAQ_Inject(const DAQ_PktHdr_t* h, int rev, const uint8_t* buf, u_int32_t len)
{
...
int DAQ_ModifyFlowOpaque(const DAQ_PktHdr_t *hdr, u_int32_t opaque)
{
    DAQ_ModFlow_t mod;
...
int DAQ_ModifyFlowHAState(const DAQ_PktHdr_t *hdr, const void *data, u_int32_t length)
{
#ifdef REG_TEST
    return 0;
#else
