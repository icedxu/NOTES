...
# endif
...
#ifdef TARGET_BASED
    u_int32_t max_attribute_hosts;    /* config max_attribute_hosts */
    u_int32_t max_attribute_services_per_host;    /* config max_attribute_services_per_host */
    u_int32_t max_metadata_services;  /* config max_metadata_services */
#endif
#if defined(FEAT_OPEN_APPID)

    u_int32_t max_metadata_appid;
#endif /* defined(FEAT_OPEN_APPID) */
    ...
    u_int32_t so_rule_memcap;
    u_int32_t paf_max;          /* config paf_max */
    char *cs_dir;
    bool ha_peer;
    char *ha_out;
    char *ha_in;
...
#ifdef SNORT_RELOAD
typedef u_int32_t snort_reload_t;
...
static inline u_int32_t ScIpv6FragTimeout(void)
{
    return snort_conf->ipv6_frag_timeout;
}

static inline u_int32_t ScIpv6MaxFragSessions(void)
{
...
static inline u_int32_t ScPafMax (void)
{
...
static inline u_int32_t ScEventLogId(void)
{
...
{
	LogMessage("[PT] ScProfilePreprocs\n");
...
#ifdef TARGET_BASED
static inline u_int32_t ScMaxAttrHosts(SnortConfig *sc)
{
    return sc->max_attribute_hosts;
}

static inline u_int32_t ScMaxAttrServicesPerHost(void)
{
...
static inline void EnableInternalEvent(RateFilterConfig *config, u_int32_t sid)
{
    if (config == NULL)
        return;
...
static inline int InternalEventIsEnabled(RateFilterConfig *config, u_int32_t sid)
{
    if (config == NULL)
        return 0;
...
static inline int ScIsPreprocEnabled(u_int32_t preproc_id, tSfPolicyId policy_id)
{
    SnortPolicy *policy;
...
static inline u_int32_t ScSoRuleMemcap(void)
{
