...
typedef struct _MagicData
{
    ...
    u_int32_t offset;             /* pattern search start offset */
...
typedef struct _FileContext
{
    ...
    u_int32_t current_data_len;
    File_Verdict verdict;
    bool suspend_block_verdict;
    FileState file_state;
    u_int32_t file_id;
    u_int32_t file_config_version;
} FileContext;
...
/*File properties*/
void file_name_set (FileContext *context, uint8_t *file_name, u_int32_t name_size, bool save_in_context);
int file_name_get (FileContext *context, uint8_t **file_name, u_int32_t *name_size);
