 ...
 */
int file_resume_block_add_file(void *pkt, u_int32_t file_sig, u_int32_t timeout,
        File_Verdict verdict, u_int32_t file_type_id, uint8_t *signature, uint16_t cli_port, uint16_t srv_port, bool create_pinhole)
{
    FileHashKey hashKey;
    ...
    if ( timeout == 0  && verdict == 0 && file_type_id == 0)
    {
        ...
        if (!context)
            return -1;
        timeout = (u_int32_t)file_config->file_block_timeout;
...
File_Verdict file_resume_block_check(void *pkt, u_int32_t file_sig)
{
    File_Verdict verdict = FILE_VERDICT_UNKNOWN;
    sfaddr_t* srcIP;
    sfaddr_t* dstIP;
