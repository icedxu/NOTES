...
typedef u_int32_t (*File_policy_callback_func) (void* ssnptr, int16_t app_id, bool upload);
typedef File_Verdict (*File_type_callback_func) (void* p, void* ssnptr,
        u_int32_t file_type_id, bool upload, u_int32_t file_id);
...
typedef int (*Get_file_name_func) (void* ssnptr, uint8_t **file_name, u_int32_t *name_len);
...
typedef void (*Set_file_name_func) (void* ssnptr, uint8_t *, u_int32_t, bool);
...
typedef void* (*Init_log_mempool_func)(u_int32_t email_hdrs_log_depth, u_int32_t memcap,  void *mempool, const char *preproc_name);

typedef int (*File_resume_block_add_file_func)(void *pkt, u_int32_t file_sig,
        u_int32_t timeout, File_Verdict verdict, u_int32_t file_type_id, uint8_t *signature,
        uint16_t cli_port, uint16_t srv_port, bool create_pinhole);
typedef File_Verdict (*File_resume_block_check_func)(void *pkt, u_int32_t file_sig);
typedef u_int32_t (*Str_to_hash_func)(uint8_t *str, int length );
...
typedef u_int32_t (*Get_file_type_id)(void *);
typedef u_int32_t (*Get_new_file_instance)(void *);
...
typedef void *(*File_cache_update_entry_func) (struct _FileCache *fileCache, void* p, uint64_t file_id,
        uint8_t *file_name, u_int32_t file_name_size,  uint64_t file_size);
typedef int (*File_segment_process_func)( struct _FileCache *fileCache, void* p, uint64_t file_id,
        ...
        bool upload);
typedef struct _FileCache * (*File_cache_create_func)(uint64_t memcap, u_int32_t cleanup_files);
...
typedef FileCharEncoding (*Get_character_encoding)(uint8_t *, u_int32_t);
...
typedef struct _file_api
{
     ...
     *    u_int32_t *name_len: address to save file name length
     * Returns
     ...
     *    u_int32_t name_len: file name length
     ...
     *    u_int32_t: maximal number of files pruned when memcap is reached
     * Returns:
