 ...
 */
static inline u_int32_t get_max_files_from_memcap (uint64_t memcap)
{
    /* Per file cost*/
    u_int32_t per_file_cost = sizeof(FileContext) + sizeof(FileKey) + sizeof(FileEntry);
...
/* Create file cache */
FileCache *file_cache_create(uint64_t memcap, u_int32_t cleanup_files)
{
        ...
        bool upload)
{
    ...
    if (!fileEntry->context)
    {
        ...
        fileEntry->context->file_id = (u_int32_t)file_id;
    }
