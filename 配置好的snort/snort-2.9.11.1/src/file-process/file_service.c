...
static u_int32_t file_config_version = 0;
...
/*File properties*/
static int get_file_name(void* ssnptr, uint8_t **fname, u_int32_t *name_size);
...
static void set_file_name(void* ssnptr, uint8_t * fname, u_int32_t name_size,
...
static u_int32_t str_to_hash(uint8_t *str, int length );
...
static u_int32_t get_file_type_id(void *ssnptr);
static u_int32_t get_new_file_instance(void *ssnptr);
...
extern FileEntry *file_cache_get(FileCache *fileCache, void* p, uint64_t file_id,
    bool can_create);
static FileCharEncoding get_character_encoding(uint8_t *buffer, u_int32_t length);
...
int file_eventq_add(u_int32_t gid, u_int32_t sid, char *msg, RuleType type)
{
    OptTreeNode *otn;
    RuleTreeNode *rtn;
...
static inline void add_file_to_block(Packet *p, File_Verdict verdict,
        u_int32_t file_type_id, uint8_t *signature)
{
    uint8_t *buf = NULL;
    u_int32_t len = 0;
    u_int32_t type = 0;
    ...
    if (GetHttpUriData(p->ssnptr, &buf, &len, &type))
    {
        ...
        file_resume_block_add_file(p, file_sig,
                (u_int32_t)file_config->file_block_timeout,
 ...
 *
 *  PARAMS:
 *      u_int32_t - session flags passed in to callback.
 *
 *  RETURNS:
...
static bool check_paf_abort(void* ssn)
{
    u_int32_t flags = session_api->get_session_flags(ssn);
