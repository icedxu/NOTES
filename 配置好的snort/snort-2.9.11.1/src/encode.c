...
typedef ENC_STATUS (*Updater)(Packet*, Layer*, u_int32_t* len);
...
static const uint8_t* Encode_Packet(
    EncState* enc, const Packet* p, u_int32_t* len);
...
const uint8_t* Encode_Reject(
    EncodeType type, EncodeFlags flags, const Packet* p, u_int32_t* len)
{
    EncState enc;
...
const uint8_t* Encode_Response(
    EncodeType type, EncodeFlags flags, const Packet* p, u_int32_t* len,
    const uint8_t* payLoad, u_int32_t payLen
) {
    EncState enc;
...
int Encode_Format_With_DAQ_Info (
    ...
    const DAQ_PktHdr_t* phdr, u_int32_t opaque)
...
static ENC_STATUS Eth_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    *len += lyr->length;
    return ENC_OK;
}
...
static ENC_STATUS FPath_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    *len += lyr->length;
    return ENC_OK;
}
...
static ENC_STATUS IP4_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
static ENC_STATUS ICMP4_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
static ENC_STATUS UDP_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
static ENC_STATUS TCP_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
static ENC_STATUS IP6_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
static ENC_STATUS Opt6_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    int i = lyr - p->layers;
    *len += lyr->length;
...
static ENC_STATUS ICMP6_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    IcmpHdr* h = (IcmpHdr*)(lyr->start);
...
static ENC_STATUS GTP_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
...
#if 0
static ENC_STATUS Top_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    *len += lyr->length + p->dsize;
    return ENC_OK;
}
...
static ENC_STATUS XXX_Update (Packet* p, Layer* lyr, u_int32_t* len)
{
    *len += lyr->length;
    return ENC_OK;
}
