...
static inline int SnortDecompressZlib(decompress_state_t *state, uint8_t *input, u_int32_t input_len,
                               uint8_t *output, u_int32_t output_bufsize, u_int32_t *output_len)
{
    z_streamp zlib_stream = (z_streamp) state->lib_info;
    int zlib_ret;
...
/* This is the function that decompresses data.
   ...
   Arguments:
     ...
     u_int32_t input_len      => length of input to decompress.
                                ignored if "input" is set to NULL.
     uint8_t *output         => pointer to buffer where decompressed output will be stored.
     u_int32_t output_bufsize => available space in output buffer.
     u_int32_t *output_len    => gets set to the actual amount of output generated.
   Returns:
...
*/
ENGINE_LINKAGE int SnortDecompress(void *state, uint8_t *input, u_int32_t input_len,
                    uint8_t *output, u_int32_t output_bufsize, u_int32_t *output_len)
{
...
int main (int argc, char *argv[])
{
    ...
    while (bytes_read > 0)
    {
        u_int32_t output_bufsize = sizeof(output_buffer);
