...
static u_int32_t getNumberTailingZerosInBitmask(u_int32_t bitmask);
 ...
 */
int extractValueInternal(void *p, ByteData *byteData, u_int32_t *value, const uint8_t *cursor)
{
 ...
 */
ENGINE_LINKAGE int checkValue(void *p, ByteData *byteData, u_int32_t value, const uint8_t *cursor)
{
    switch (byteData->op)
    {
        case CHECK_EQ:
 ...
 * Return 0 if check is not true.
 */
ENGINE_LINKAGE int checkValue_Bytemath(void *p, ByteData *byteData, u_int32_t value, const uint8_t *cursor)
{
    if (!value)
       return 0;
