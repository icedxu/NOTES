...
static u_int32_t so_rule_memory = 0;
...
int DynamicSetRuleData(void *p, void *data, u_int32_t sid, SessionDataFree sdf)
{
    Packet *pkt = (Packet *)p;
...
void * DynamicGetRuleData(void *p, u_int32_t sid)
{
    Packet *pkt = (Packet *)p;
...
static int setFlowId(const void* p, u_int32_t id)
{
...
void *AddPreprocessorAllPolicies(struct _SnortConfig *sc, void (*pp_func)(void *, void *),
                                 uint16_t priority, u_int32_t preproc_id, u_int32_t proto_mask)
{
    SnortPacketProcessFunc preprocessorFunc = (SnortPacketProcessFunc)pp_func;
...
int DynamicEnablePreprocessor(void *p, u_int32_t preprocId)
{
...
static int DynamicEncodeFormat (u_int32_t f, const void* p, void *c, int t)
{
...
#ifdef ACTIVE_RESPONSE
void DynamicSendBlockResponseMsg(void *p, const uint8_t* buffer, u_int32_t buffer_len, unsigned flags)
{
...
void DynamicActiveResponseMsg(void *p, const uint8_t* buf, u_int32_t blen, unsigned flags)
{
    EncodeFlags df = (SND_BLK_RESP_FLAG_DO_CLIENT) ? 0: ENC_FLAG_FWD;

    Active_UDPInjectData((Packet *)p, df , buf, blen);
}
void DynamicActiveInjectData(void *p, u_int32_t flags, const uint8_t *buf, u_int32_t blen)
{
...
u_int32_t DynamicGetSnortInstance(void)
{
...
int DynamicSCPreallocMessageTX(u_int32_t length, SCMsgHdr **hdr_ptr, uint8_t **msg_ptr, void **msg_handle)
{
    return SideChannelPreallocMessageTX(length, hdr_ptr, msg_ptr, msg_handle);
}

int DynamicSCEnqueueMessageTX(SCMsgHdr *hdr, const uint8_t *msg, u_int32_t length, void *msg_handle, SCMQMsgFreeFunc msgFreeFunc)
{
...
static int userGroupIdGet(void *ssnptr, u_int32_t *userId, u_int32_t *realmId, unsigned *groupIdArray, unsigned groupIdArrayLen)
{
...
static void endSSLSSnLogData(void *ssnptr, u_int32_t ssl_flow_messages, uint64_t ssl_flow_flags)
{
...
static u_int32_t dummyProduceHAState(void *lwssn, uint8_t *buf)
{
    return 0;
}

static u_int32_t dummyConsumeHAState(void *lwssn, const uint8_t *buf, uint8_t length, uint8_t proto, const struct in6_addr* ip, uint16_t initiatorPort)
{
    return 0;
}
...
static u_int32_t dummyGetDNSTTL(struct AppIdData *session)
{
    return 0;
}
