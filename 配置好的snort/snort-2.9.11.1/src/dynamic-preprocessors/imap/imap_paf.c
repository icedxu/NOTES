...
typedef struct _ImapPafData
{
    ...
    bool end_of_data;
//    u_int32_t length;            TODO -- parse and add literal length
} ImapPafData;
...
static bool parse_literal_length(const uint8_t ch, u_int32_t *len)
{
 ...
 *    u_int32_t len - length of payload data
 *    u_int32_t * fp- pointer to set flush point
 *
 * RETURNS:
...
static PAF_Status imap_paf_server(ImapPafData *pfdata,
        const uint8_t* data, u_int32_t len, u_int32_t* fp)
{
    u_int32_t i;
    u_int32_t flush_len = 0;
    u_int32_t boundary_start = 0;
        ...
        const uint8_t* data, u_int32_t len, u_int32_t* fp)
{
    ...
    if (pch != NULL)
    {
        DEBUG_WRAP(DebugMessage(DEBUG_IMAP, "IMAP PAF: Flushing client"
                                    " data!\n"););
        *fp = (u_int32_t)(pch - (const char*)data) + 1;
        return PAF_FLUSH;
    }
    return PAF_SEARCH;
}
 ...
 *    const uint8_t *data - payload data to inspect
 *    u_int32_t len - length of payload data
 *    u_int32_t flags- flags to check whether client or server
 *    u_int32_t * fp- pointer to set flush point
 *    u_int32_t * fp_eoh - pointer to set header flush point
 *
 * Returns:
