...
void IMAP_Set_flow_id( void *app_data, u_int32_t fid );
        ...
        if (resp_found > 0)
        {
            ...
            if(imap_ssn->state == STATE_DATA)
            {
                ...
                else
                {
                    ...
                    {
                        len = (u_int32_t)_dpd.SnortStrtoul((const char *)(body_start + 1), &eptr, 10);
                        if (*eptr != '}')
                        {
                            imap_ssn->state = STATE_UNKNOWN;
                        }
...
int IMAP_GetFilename(void *data, uint8_t **buf, u_int32_t *len, u_int32_t *type)
{
