...
static inline IPdecision GetReputation(IPrepInfo *, SFSnortPacket *, u_int32_t *);
#ifdef SHARED_REP
...
static int Reputation_MgmtInfo(uint16_t type, const uint8_t *data,
    u_int32_t length, void **new_config, char *statusBuf, int statusBufLen)
{
...
static int Reputation_Lookup(uint16_t type, const uint8_t *data, u_int32_t length, void **new_config,
        char *statusBuf, int statusBufLen)
{
    ...
    if (length != (u_int32_t)ntohs(msg_hdr->length))
    {
        return -1;
    }
...
static int Reputation_PreControl(uint16_t type, const uint8_t *data, u_int32_t length, void **new_config,
        char *statusBuf, int statusBufLen)
{
    ReputationConfig *pDefaultPolicyConfig = NULL;
...
void SetupReputationUpdate(u_int32_t updateInterval)
{
...
static inline void createZones(u_int32_t *ingressZone, u_int32_t *egressZone, SFSnortPacket *p)
{
    const u_int32_t zone_base = 0xF700;
...
static inline IPdecision GetReputation(  IPrepInfo * repInfo,
        SFSnortPacket *p, u_int32_t *listid)
{
    IPdecision decision = DECISION_NULL;
    uint8_t *base ;
    ListInfo *listInfo;
#ifdef SHARED_REP
    u_int32_t ingressZone = 0;
    u_int32_t egressZone = 0;
#ifdef DAQ_PKTHDR_UNKNOWN
    if (p->pkt_header)
    {
...
static inline void ReputationProcess(SFSnortPacket *p)
{
    ...
    {
        u_int32_t flags = SSNFLAG_DETECTION_DISABLED;
