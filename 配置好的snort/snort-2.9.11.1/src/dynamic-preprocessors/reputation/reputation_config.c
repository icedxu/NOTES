 ...
 */
static void IpListInit(u_int32_t,ReputationConfig *config);
 ...
 * u_int32_t the memcap value set in configuration
 *
 * RETURNS: estimated memory size.
 *********************************************************************/
u_int32_t estimateSizeFromEntries(u_int32_t num_entries, u_int32_t memcap)
{
    uint64_t size;
 ...
 *
 * RETURNS:
 *    u_int32_t: segment size
 *********************************************************************/
u_int32_t GetSegmentSizeFromFileList(ShmemDataFileList** file_list, int file_count)
{
    int numlines;
    int totalLines = 0;
    int i;
...
static void IpListInit(u_int32_t maxEntries, ReputationConfig *config)
{
    uint8_t *base;
    ListInfo *whiteInfo;
    ListInfo *blackInfo;
...
static int AddIPtoList(sfcidr_t *ipAddr,INFO ipInfo_ptr, ReputationConfig *config)
{
     ...
     */
    u_int32_t usageBeforeAdd;
    u_int32_t usageAfterAdd;
...
void ParseReputationArgs(ReputationConfig *config, u_char* argp)
{
    ...
    while ( cur_sectionp )
    {
        ...
        else if ( !strcasecmp( cur_tokenp, REPUTATION_SHARED_REFRESH_KEYWORD ))
        {
            ...
            config->sharedMem.updateInterval = (u_int32_t) value;
