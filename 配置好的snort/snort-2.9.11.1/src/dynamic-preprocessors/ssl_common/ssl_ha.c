...
typedef struct
{
    u_int32_t update_messages_received;
    u_int32_t delete_messages_received;
    u_int32_t update_messages_sent;
    u_int32_t delete_messages_sent;
} SSLHAStats;
...
static void SSLHADebugParse(const char *desc, const uint8_t *data, u_int32_t length,
        volatile int *debug_flag, HADebugSessionConstraints *info)
{
}

static int SSLDebugHA(uint16_t type, const uint8_t *data, u_int32_t length, void **new_context, char *statusBuf, int statusBuf_len)
{
...
int RegisterSSLHAFuncs(u_int32_t preproc_id, uint8_t subcode, u_int32_t size,
...
void UnregisterSSLHAFuncs(u_int32_t preproc_id, uint8_t subcode)
{
    SSLHAFuncsNode *node;
    int i;
...
static inline int DeserializeSSLPreprocData(uint8_t event, uint8_t preproc_id,
                                         uint8_t subcode, const uint8_t *data, u_int32_t length)
{
    SSLHAFuncsNode *node;
    int i;
...
static int ConsumeSSLHAMessage(const uint8_t *msg, u_int32_t msglen)
{
    MsgHeader *msg_hdr;
...
static u_int32_t WriteSSLHAMessageHeader(uint8_t event, uint16_t msglen, uint8_t *msg)
{
    MsgHeader *msg_hdr;
    u_int32_t offset;
...
static u_int32_t WriteSSLPreprocDataRecord(SSLHAFuncsNode *node, uint8_t *msg, void *data, int size)
{
    RecordHeader *rec_hdr;
    PreprocDataHeader *psd_hdr;
    u_int32_t offset;
...
static u_int32_t CalculateSSLHAMessageSize(uint8_t event, int index, u_int32_t ssl_size)
{
    SSLHAFuncsNode *node;
    u_int32_t msg_size;
...
static u_int32_t GenerateSSLHAUpdateMessage(uint8_t *msg, u_int32_t msg_size, int index, uint8_t event, void *data, u_int32_t size)
{
    SSLHAFuncsNode *node;
    u_int32_t offset;
    PROFILE_VARS;
...
#ifdef SIDE_CHANNEL
static void SendSSLSCUpdateMessage(int index, u_int32_t msg_size, uint8_t event, void *data, u_int32_t size)
{
    SCMsgHdr *schdr;
    void *msg_handle;
    uint8_t *msg;
...
void SSLProcessHA(int ssl_index, bool update, void *data, u_int32_t ssl_size)
{
    u_int32_t msg_size;
...
#ifdef SIDE_CHANNEL
static int SSLHASCMsgHandler(SCMsgHdr *hdr, const uint8_t *msg, u_int32_t msglen)
{
    int rval;
    PROFILE_VARS;
