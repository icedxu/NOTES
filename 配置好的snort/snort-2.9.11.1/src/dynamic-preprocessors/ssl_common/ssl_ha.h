...
typedef u_int32_t (*SSLHAProducerFunc)(uint8_t *buf, void *data_to_copy, u_int32_t size);
typedef int (*SSLHAConsumerFunc)(const uint8_t *data, u_int32_t length);

int RegisterSSLHAFuncs(u_int32_t preproc_id, uint8_t subcode, u_int32_t size, SSLHAProducerFunc produce, SSLHAConsumerFunc consume, SSLHAConsumerFunc deletion);
void UnregisterStreamHAFuncs(u_int32_t preproc_id, uint8_t subcode);
