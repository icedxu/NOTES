...
typedef void (*PP_Set_Flow_Id_Callback_Func) (void *app_data, u_int32_t flow_context);
...
typedef struct _ssl_callback_interface
{
    ...
    void (*session_free)(u_int32_t fid);
    bool (*is_session_ssl)(SFSnortPacket* p);
    int  (*get_ssl_flow_flags)(SFSnortPacket* p, void *sd, u_int32_t *ssn_flags);
