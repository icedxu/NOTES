...
{
    u_int32_t ssl_flags = SSL_decode(ptr, len, pkt_flags, 0, NULL, NULL, 0);
...
void SSL_Set_flow_id( void *app_data, u_int32_t fid );
...
#ifdef DEBUG_MSGS
static void SSL_PrintFlags(u_int32_t flags)
{
...
static void SSL_UpdateCounts(const u_int32_t new_flags)
{
...
static inline bool SSLPP_is_encrypted(u_int32_t ssl_flags, SFSnortPacket *packet)
{
...
static inline void SSLPP_process_other(
        SSL_SsnData *sd, u_int32_t new_flags, SFSnortPacket *packet)
{
    SSLPP_config_t *config = NULL;
