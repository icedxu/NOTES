...
static uint8_t * DCE2_Smb2GetFileName(const uint8_t *data,
        u_int32_t data_len, uint16_t *file_name_len)
{
    uint8_t *fname = NULL;
    int i;
...
static inline u_int32_t Smb2Tid(const Smb2Hdr *hdr)
{
...
{
    u_int32_t x = (u_int32_t)(uintptr_t)a;
    u_int32_t y = (u_int32_t)(uintptr_t)b;
...
static inline void DCE2_Smb2InsertTid(DCE2_SmbSsnData *ssd, const u_int32_t tid,
...
static inline void DCE2_Smb2RemoveTid(DCE2_SmbSsnData *ssd, const u_int32_t tid)
{
...
static inline void DCE2_Smb2ProcessFileData(DCE2_SmbSsnData *ssd, const uint8_t *file_data,
        u_int32_t data_size, bool upload)
{
        ...
        Smb2ReadResponseHdr *smb_read_hdr, const uint8_t *end)
{
    ...
    total_data_length = SmbNtohl((const u_int32_t *)&(smb_read_hdr->length));
    if (total_data_length > (u_int32_t)data_size)
        ...
        Smb2WriteRequestHdr *smb_write_hdr, const uint8_t *end)
{
    ...
    ssd->ftracker.tracker.file.file_offset += data_size;
    total_data_length = SmbNtohl((const u_int32_t *)&(smb_write_hdr->length));
    if (total_data_length > (u_int32_t)data_size)
        ssd->pdu_state = DCE2_SMB_PDU_STATE__RAW_DATA;
}
...
DCE2_SmbVersion DCE2_Smb2Version(const SFSnortPacket *p)
{
            ...
            (p->payload_size > sizeof(NbssHdr) + sizeof(DCE2_SMB_ID)))
    {
        ...
        u_int32_t smb_version_id = SmbId((SmbNtHdr *)smb_hdr);
