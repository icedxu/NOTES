...
typedef struct _Smb2SyncHdr
{
    ...
    u_int32_t tree_id;         /* identifies the tree connect for the command */
    uint64_t session_id;      /* identifies the established session for the command*/
    uint8_t  signature[16] ;  /* signature of the message */
} Smb2SyncHdr;
...
typedef struct _Smb2ErrorResponseHdr
{
    ...
    u_int32_t byte_count;      /* The number of bytes of error_data */
...
typedef struct _Smb2WriteRequestHdr
{
    ...
    u_int32_t length;          /* length of data being written in bytes */
    ...
    u_int32_t channel;            /* channel */
    u_int32_t remaining_bytes;    /* subsequent bytes the client intends to write*/
...
typedef struct _Smb2WriteResponseHdr
{
    ...
    uint16_t reserved;        /* reserved */
    u_int32_t count;           /* The number of bytes written */
    u_int32_t remaining;       /* MUST be 0*/
    uint16_t write_channel_info_offset;      /* channel data info */
    uint16_t write_channel_info_length;      /* channel data info */
...
typedef struct _Smb2ReadRequestHdr
{
    ...
    uint8_t  flags;           /* Flags */
    u_int32_t length;          /* length of data to read from the file */
    uint64_t offset;          /* offset in the destination file */
    uint64_t fileId_persistent;  /* fileId that is persistent */
    uint64_t fileId_volatile;    /* fileId that is volatile */
    u_int32_t minimum_count;      /* The minimum # of bytes to be read */
    u_int32_t channel;            /* channel */
    u_int32_t remaining_bytes;    /* subsequent bytes the client intends to read*/
...
typedef struct _Smb2ReadResponseHdr
{
    ...
    u_int32_t length;         /* The number of bytes being returned in response */
    u_int32_t remaining;      /* The number of data being sent on the channel*/
...
typedef struct _Smb2SetInfoRequestHdr
{
    ...
    u_int32_t buffer_length;    /* buffer length */
    uint16_t buffer_offset;    /* buffer offset */
    uint16_t reserved;         /* reserved */
    u_int32_t additional_info;  /* additional information */
    uint64_t fileId_persistent;/* fileId that is persistent */
    uint64_t fileId_volatile;  /* fileId that is volatile */
...
typedef struct _Smb2CreateRequestHdr
{
    ...
    u_int32_t impersonation_level;     /* */
    uint64_t smb_create_flags;        /* should be 0 */
    uint64_t reserved;                /* can be any value */
    u_int32_t desired_access;          /*  */
    u_int32_t file_attributes;         /* */
    u_int32_t share_access;            /* READ WRITE DELETE etc */
    u_int32_t create_disposition;      /* actions when file exists*/
    u_int32_t create_options;          /* options for creating file*/
    uint16_t name_offset;             /* file name offset from SMB2 header */
    uint16_t name_length;             /* length of file name */
    u_int32_t create_contexts_offset;  /* offset of contexts from beginning of header */
    u_int32_t create_contexts_length;  /* length of contexts */
...
typedef struct _Smb2CreateResponseHdr
{
    ...
    u_int32_t create_action;           /* action taken, values limited */
    ...
    u_int32_t file_attributes;         /* attributes of the file*/
    u_int32_t reserved2;               /* */
    uint64_t fileId_persistent;       /* fileId that is persistent */
    uint64_t fileId_volatile;         /* fileId that is volatile */
    u_int32_t create_contexts_offset;  /*  */
    u_int32_t create_contexts_length;  /*  */
...
typedef struct _Smb2CloseRequestHdr
{
    ...
    u_int32_t reserved;                /* can be any value */
    uint64_t fileId_persistent;       /* fileId that is persistent */
    uint64_t fileId_volatile;         /* fileId that is volatile */
} Smb2CloseRequestHdr;
...
typedef struct _Smb2TreeConnectResponseHdr
{
    ...
    uint8_t  reserved;                /* reserved */
    u_int32_t share_flags;             /* properties for this share*/
    u_int32_t capabilities;            /* various capabilities for this share */
    u_int32_t maximal_access;          /* maximal access for the user */
