...
static inline SFSnortPacket * DCE2_CoGetSegRpkt(DCE2_SsnData *, const uint8_t *, u_int32_t);
...
static DCE2_Ret DCE2_CoSegEarlyRequest(DCE2_CoTracker *, const uint8_t *, u_int32_t);
                              ...
                              const DceRpcCoHdr *co_hdr, const uint8_t *frag_ptr, uint16_t frag_len)
{
    ...
    u_int32_t size = (frag_len < DCE2_CO__MIN_ALLOC_SIZE) ? DCE2_CO__MIN_ALLOC_SIZE : frag_len;
...
static void DCE2_CoEarlyReassemble(DCE2_SsnData *sd, DCE2_CoTracker *cot)
{
    ...
    {
        u_int32_t bytes = DCE2_BufferLength(frag_buf);
        u_int32_t seg_bytes = 0;
    ...
    {
        uint16_t hdr_size = sizeof(DceRpcCoHdr) + sizeof(DceRpcCoRequest);
        u_int32_t bytes = DCE2_BufferLength(cot->cli_seg.buf);
                                      ...
                                      DCE2_CoRpktType co_rtype, DCE2_RpktType *rtype)
{
    ...
    u_int32_t frag_len = 0, seg_len = 0;
    SFSnortPacket *rpkt = NULL;
...
static DCE2_Ret DCE2_CoSegEarlyRequest(DCE2_CoTracker *cot,
                                       const uint8_t *seg_ptr, u_int32_t seg_len)
{
