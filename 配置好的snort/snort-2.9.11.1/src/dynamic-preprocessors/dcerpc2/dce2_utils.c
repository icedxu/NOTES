 ...
 *
 ********************************************************************/
DCE2_Buffer * DCE2_BufferNew(u_int32_t initial_size, u_int32_t min_add_size, DCE2_MemType mem_type)
{
...
DCE2_Ret DCE2_BufferAddData(DCE2_Buffer *buf, const uint8_t *data,
    u_int32_t data_len, u_int32_t data_offset, DCE2_BufferMinAddFlag mflag)
{
    DCE2_Ret status;
    ...
    if (buf->data == NULL)
    {
        u_int32_t size = data_offset + data_len;
    ...
    {
        uint8_t *tmp;
        u_int32_t new_size = data_offset + data_len;
 ...
 *
 ********************************************************************/
DCE2_Ret DCE2_BufferMoveData(DCE2_Buffer *buf, u_int32_t data_offset,
                             const uint8_t *move, u_int32_t move_len)
{
    DCE2_Ret status;
    uint8_t *offset, *end;
...
DCE2_Ret DCE2_HandleSegmentation(DCE2_Buffer *seg_buf, const uint8_t *data_ptr,
    uint16_t data_len, u_int32_t need_len, uint16_t *data_used)
{
    u_int32_t copy_len;
    DCE2_Ret status;
