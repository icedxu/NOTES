...
static int DCE2_ScCheckTransport(void *data)
{
    ...
    {
        if (*((u_int32_t *)&sc->smb_ports[i]) ||
            *((u_int32_t *)&sc->tcp_ports[i]) ||
            *((u_int32_t *)&sc->udp_ports[i]) ||
            *((u_int32_t *)&sc->http_proxy_ports[i]) ||
            *((u_int32_t *)&sc->http_server_ports[i]) ||
            *((u_int32_t *)&sc->auto_smb_ports[i]) ||
            *((u_int32_t *)&sc->auto_tcp_ports[i]) ||
            *((u_int32_t *)&sc->auto_udp_ports[i]) ||
            *((u_int32_t *)&sc->auto_http_proxy_ports[i]) ||
            *((u_int32_t *)&sc->auto_http_server_ports[i]))
        {
            return 0;
        }
    }
...
static DCE2_Ret DCE2_ScCheckPortOverlap(const DCE2_ServerConfig *sc)
{
    ...
    for (i = 0; i < sizeof(sc->smb_ports) - 3; i += 4)
    {
        ...
        u_int32_t overlap = *((u_int32_t *)&sc->smb_ports[i]) & *((u_int32_t *)&sc->tcp_ports[i]);
        u_int32_t cached;
        ...
        cached = *((u_int32_t *)&sc->smb_ports[i]) | *((u_int32_t *)&sc->tcp_ports[i]);
        overlap = *((u_int32_t *)&sc->http_proxy_ports[i]) & cached;
        ...
        cached |= *((u_int32_t *)&sc->http_proxy_ports[i]);
        overlap = *((u_int32_t *)&sc->http_server_ports[i]) & cached;
    ...
    switch (int_type)
    {
        ...
        case DCE2_INT_TYPE__INT32:
            *(int32_t *)int_value = (int32_t)value;
            break;
        case DCE2_INT_TYPE__UINT32:
            *(u_int32_t *)int_value = (u_int32_t)value;
            break;
        case DCE2_INT_TYPE__INT64:
