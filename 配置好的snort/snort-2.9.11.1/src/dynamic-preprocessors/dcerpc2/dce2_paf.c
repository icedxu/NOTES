...
static inline bool DCE2_PafSmbIsValidNetbiosHdr(u_int32_t, bool);
static inline bool DCE2_PafAbort(void *, uint64_t);
static PAF_Status DCE2_SmbPaf(void *, void **, const uint8_t *, u_int32_t, uint64_t *, u_int32_t *, u_int32_t *);
static PAF_Status DCE2_TcpPaf(void *, void **, const uint8_t *, u_int32_t, uint64_t *, u_int32_t *, u_int32_t *);
 ...
 *  u_int32_t - flags passed in to callback.
 ...
 *
 * Arguments:
 *  u_int32_t - the 4 bytes of the NetBIOS header
 ...
 *
 *********************************************************************/
static inline bool DCE2_PafSmbIsValidNetbiosHdr(u_int32_t nb_hdr, bool junk)
{
 ...
 *  u_int32_t - length of payload data
 *  u_int32_t - flags to check whether client or server
 *  u_int32_t * - pointer to set flush point
 *  u_int32_t * - pointer to set header flush point
 *
 * Returns:
...
PAF_Status DCE2_SmbPaf(void *ssn, void **user, const uint8_t *data,
        u_int32_t len, uint64_t *flags, u_int32_t *fp, u_int32_t *fp_eoh)
{
    ...
    while (n < len)
    {
        ...
        switch (ss->state)
        {
            ...
            case DCE2_PAF_SMB_STATES__3:
                ...
                if (DCE2_PafSmbIsValidNetbiosHdr((u_int32_t)ss->nb_hdr, false))
                {
                    nb_hdr = htonl((u_int32_t)ss->nb_hdr);
            ...
            case DCE2_PAF_SMB_STATES__7:
                ...
                if (!DCE2_PafSmbIsValidNetbiosHdr((u_int32_t)(ss->nb_hdr >> 32), true))
                {
                    DEBUG_WRAP(DCE2_DebugMsg(DCE2_DEBUG__PAF, "Invalid NetBIOS header - "
                                             "staying in State 7.\n"));
                    break;
                }
                if (((u_int32_t)ss->nb_hdr != DCE2_SMB_ID)
                        && ((u_int32_t)ss->nb_hdr != DCE2_SMB2_ID))
                {
                ...
                nb_hdr = htonl((u_int32_t)(ss->nb_hdr >> 32));
                nb_len = NbssLen((const NbssHdr *)&nb_hdr);
                *fp = (nb_len + sizeof(NbssHdr) + n) - ss->state;
                DEBUG_WRAP(DCE2_DebugMsg(DCE2_DEBUG__PAF,
                            "Setting flush point: %u\n", *fp));
