...
static u_int32_t DCE2_IfaceHash(void *);
static u_int32_t DCE2_OpnumHash(void *);
static u_int32_t DCE2_ByteTestHash(void *);
static u_int32_t DCE2_ByteJumpHash(void *);
...
static inline void * DCE2_AllocFp(u_int32_t);
            ...
            case 0:
                {
                    ...
                    iface_data->iface.time_low = (u_int32_t)time_low;
                }
...
static inline void * DCE2_AllocFp(u_int32_t size)
{
        ...
        int direction, FPContentInfo **info)
{
    ...
    else
    {
        ...
        memcpy(&big_content[index], &time32, sizeof(u_int32_t));
        time32 = DceRpcNtohl(&iface_data->iface.time_low,
                DCERPC_BO_FLAG__LITTLE_ENDIAN);
        memcpy(&little_content[index], &time32, sizeof(u_int32_t));
...
static int DCE2_ByteTestEval(void *pkt, const uint8_t **cursor, void *data)
{
    ...
    switch (bt_data->num_bytes)
    {
        ...
        case 2:
            ...
            break;
        case 4:
            pkt_value = DceRpcNtohl((u_int32_t *)bt_ptr, byte_order);
            DEBUG_WRAP(DCE2_DebugMsg(DCE2_DEBUG__ROPTIONS,
                        "Got 4 bytes: %u.\n", pkt_value));
            break;
        default:
...
static int DCE2_ByteJumpEval(void *pkt, const uint8_t **cursor, void *data)
{
    ...
    switch (bj_data->num_bytes)
    {
        ...
        case 2:
            ...
            break;
        case 4:
            jmp_value = DceRpcNtohl((u_int32_t *)bj_ptr, byte_order);
            DEBUG_WRAP(DCE2_DebugMsg(DCE2_DEBUG__ROPTIONS,
                        "Got 4 bytes: %u.\n", jmp_value));
            break;
        default:
 ...
 *
 ********************************************************************/
static u_int32_t DCE2_IfaceHash(void *data)
{
    u_int32_t a, b, c;
 ...
 *
 ********************************************************************/
static u_int32_t DCE2_OpnumHash(void *data)
{
    u_int32_t a = 0, b = 0, c = 0;
    DCE2_OpnumData *odata = (DCE2_OpnumData *)data;
 ...
 *
 ********************************************************************/
static u_int32_t DCE2_ByteTestHash(void *data)
{
    u_int32_t a, b, c;
 ...
 *
 ********************************************************************/
static u_int32_t DCE2_ByteJumpHash(void *data)
{
    u_int32_t a, b, c;
